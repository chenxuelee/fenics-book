\documentclass[a4paper,twoside,justified,notoc]{layout/fenicsbook}

% Book metadata
\title{Automated\\ \noindent Scientific\\ \noindent Computing}
\author[The FEniCS Project]{Logg, Mardal, Wells (Eds.)}
\publisher{Springer-Verlag}

% FIXME: Input package somewhere else
% Generate index
\usepackage{makeidx}
\makeindex

%------------------------------------------------------------------------------
% Begin document
\begin{document}

% Start front matter
\frontmatter

% Title page
\maketitle

% Copyright page
\input{frontmatter/copyright}

% Contents
\tableofcontents
%\listoffigures
%\listoftables

%------------------------------------------------------------------------------
% Start main matter
\mainmatter

% Start full width
\begin{fullwidth}

% Chapters

%------------------------------------------------------------------------------

% Use to test individual chapter
%\fenicsinput{logg-2}\bibliography{bibliography}\bibliographystyle{abbrvnat}\end{fullwidth}\end{document}

\input{chapters/intro/chapter.tex}
\fenicsinput{langtangen}

\part{Methodology}

\fenicsinput{kirby-7}
\fenicsinput{kirby-6}
\fenicsinput{kirby-1}
\fenicsinput{kirby-5}
\fenicsinput{logg-3}
\fenicsinput{oelgaard-2}
\fenicsinput{kirby-8}
\fenicsinput{kirby-4}

\part{Implementation}

\fenicsinput{logg-2}
\fenicsinput{logg-1}
\fenicsinput{kirby-3}
\fenicsinput{kirby-2}
\fenicsinput{wilbers}
\fenicsinput{alnes-3}
\fenicsinput{alnes-2}
\fenicsinput{alnes-1}
%\input{unfinished/hoffman-2/chapter.tex}
\fenicsinput{mardal-2}

\part{Applications}

%\fenicsinput{terrel}
\fenicsinput{kvs-1}
\fenicsinput{mortensen}
%\input{unfinished/hoffman-1/chapter.tex}
%%%\fenicsinput{hoffman-1}
\fenicsinput{selim}
%%%\fenicsinput{shan}
\fenicsinput{lopes}
\fenicsinput{kvs-2}
\fenicsinput{hentschel}
\fenicsinput{narayanan}
%%%\fenicsinput{oelgaard-1}
\fenicsinput{nikbakht}
\fenicsinput{schroll}
\fenicsinput{vynnytska}
\fenicsinput{hake}
\fenicsinput{lezar}
\fenicsinput{mardal-4}
\fenicsinput{rognes}

%------------------------------------------------------------------------------
% Start backmatter
\backmatter

% List of authors
\input{backmatter/authors}

% Bibliography
\bibliography{bibliography}
\bibliographystyle{abbrvnat}

% End full width
\end{fullwidth}

% Index
%\newcommand{\icode}[1]{\texttt{#1}} % seems necessary
%\printindex
%------------------------------------------------------------------------------
\end{document}
%------------------------------------------------------------------------------
