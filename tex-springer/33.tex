...
substances and the contraction of cells in the heart. The latter is
...
\begin{figure}[!t]
\centering
  \includegraphics{0001335366/272415_1_en_33_fig1_print.eps}
  \caption[The dyadic cleft]{(A): A diagram showing the
    ...
    model two \Ca ion channels.}\label{fig:hake:morphology}%}
...
\index{advection--diffusion equation} \index{Gouy-Chapman} \index{Nernst--Planck
  equation}
...
\begin{equation}
  ...
  J = -\sigma\left(\nabla c-2\,cE\right),\vspace*{1.5pt}
\end{equation}
...
mass, we arrive at the advection--diffusion equation,\vspace*{2.5pt}
\begin{equation}
...
\begin{array}{c@{\quad\quad}l}
    0,& \text{closed channel},\\[2pt]
    - \frac{i}{2\,F\,A},& \text{open channel},
\end{array}
\right.
\end{equation}
\looseness-1{}where $i$ is the amplitude, 2 the valence of \Ca, $F$
...
distance to the center of the cytosol. In our model, we have used \Ds~$=$~50~nm.

\begin{figure}[!b]
\centering
\includegraphics{0001335366/272415_1_en_33_fig2_print.eps}
\caption[Discrete markov models]{(A): State diagram of the
    ...
    $\beta$ and $\delta$ propensities are  constant.}\label{fig:hake:markov-models}\vspace*{-8pt}
\end{figure}
...
coefficients $\left\{ c^{n+1}_i\right\}^N_{i=1}$:\vspace*{-3pt}
\begin{equation}
  \label{eq:hake:algebraic-equation}
  \frac{1}{\Dt}\text{\bfseries\itshape M}\left(C^{n+1}-C^n\right) =  \left(-\text{\bfseries\itshape K}+\text{\bfseries\itshape E}+\sum_k\alpha^k\text{\bfseries\itshape F}^k\right)C_j^{n+1}+\sum_k c_0^k\,f^k.\vspace*{-3pt}
\end{equation}
...
and\vspace*{-3pt}\enlargethispage{6pt}
\begin{equation}
  ...
  \label{eq:hake:matrices}\vspace*{-3pt}
\end{equation}
...
vector elements given by:\vspace*{-3pt}
\begin{equation}
  \label{eq:hake:vector}
  f^k_{i}=\int_{\partial\Omega_k}\phi_i \ds.\vspace*{-3pt}
\end{equation}

The following DOLFIN code assembles the matrices and vectors
from~(\ref{eq:hake:matrices})--(\ref{eq:hake:vector}):\vspace*{-2pt}
\begin{python}
import numpy as np
from dolfin import *
...
term introduced by {\bfseries\itshape E}$_{ij}$ from~(\ref{eq:hake:matrices}). We have chosen to stabilize the system using the Streamline upwind Petrov--Galerkin (SUPG) method
\citep{BrooksHughes1982}. This method adds an upwind discontinuous
contribution to the test function in the streamline
direction~(\ref{eq:hake:advection-diffusion-weak-form-0}):\vspace*{7pt}
\begin{equation}
...
presented in the following script:\vspace*{2pt}
\begin{python}
...
\noindent In the above script, two matrices \texttt{E\_stab} and
...
from~(\ref{eq:hake:algebraic-equation}) is given by:\vspace*{1.5pt}
\begin{python}
# Model parameters
...
\noindent The time stepping scheme presented in the above code is crude, but
...
\begin{figure}[!b]
  \centering
  \includegraphics{0001335366/272415_1_en_33_fig3_print.eps}
  ...
  \caption[Error plot]{The figure shows a plot of the error versus the
    ...
    and~$0.38$.}\label{fig:hake:error_plot}\vspace*{16pt}
\end{figure}
...
\begin{figure}[!p]
  ...
  \includegraphics{0001335366/272415_1_en_33_fig4_print.eps}
...
\begin{figure}[!p]
  ...
  \caption[Concentration traces 2]{The figures show the
    ...
    analytical solution.}\label{fig:hake:traces_mesh_2}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics{0001335366/272415_1_en_33_fig6_print.eps}
...
\index{time stepping} \index{\texttt{diffsim}} \index{FEniCS Apps}

In the DOLFIN scripts above, we show how a simple continuous solver can
...
\index{Gillespie method} \index{Markov chain model}
\index{propensity function}
\index{channel transition} The stochastic evolution of the
...
\noindent The framework presented with this pseudo code can be expanded to
...
\begin{figure}[!t]
\centering
\includegraphics{0001335366/272415_1_en_33_fig7_print.tif}
\caption[Time stepping algorithm]{Diagram for the time stepping
algorithm using 3 discrete objects: \texttt{DtExpander}, \texttt{StochasticHandler}, \texttt{TStop}. The values below the
...
\label{fig:hake:time-line}\vspace*{6pt}
\end{figure}
...
fluxes. \pagebreak It also implements the field flux from the lipid bi-layer \nobreak 
