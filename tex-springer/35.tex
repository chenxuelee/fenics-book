...
preconditioned Krylov solvers for \hbox{systems} of partial differential
...
Stokes problem and finally a mixed formulation of the Hodge \hbox{Laplacian}.
...
this case\vspace*{2.5pt}
\begin{equation}
\|\mathcal{R}\|_{\mathcal{L}(H', H)} = 1 \quad \mbox{and} \quad
\|\mathcal{R}^{-1}\|_{\mathcal{L}(H, H')} = 1.\vspace*{1.5pt}
\end{equation}
...
\vspace*{10pt}
\centering
\includegraphics{0001335368/272415_1_en_35_fig1_print.eps}
...
find $u$ such that\vspace*{-1.3pt}
\begin{align}
  -\Div \Grad u &= f \mbox{ in } \Omega,  \\
  \frac{\partial u}{\partial n} &= g \mbox{ on } \partial \Omega .\vspace*{-1.3pt}
\end{align}
The corresponding variational problem is:  Find $u\in H^1 \cap L^2_0$ such that\vspace*{-1.3pt}
\[
\int_\Omega \Grad u \cdot \Grad v \dx = \int_\Omega f v \dx +
\int_{\partial \Omega} g v  \ds, \quad\foralls v \in H^1 \cap L^2_0 .\vspace*{-1.3pt}
\]
Let the linear operator $\mathcal{A}$ be defined in terms of the  bilinear form,\vspace*{-1.3pt}
\[
(\mathcal{A} u, v) =  \int_\Omega \Grad u \cdot \Grad v \dx.\vspace*{-1.3pt}
\]
...
First of all, the ML preconditioner is constructed as follows,\vspace*{-1.5pt}
\begin{python}
class ML(block_base):
...
linear algebra interface in DOLFIN:\vspace*{2pt}
\begin{python}
...
\noindent The intermediate variables called alphas and betas can then be used to
...
\noindent The following code shows the implementation of a Poisson problem
...
does therefore have mean value zero.\vspace*{2pt}
\begin{python}
...
\vspace*{5pt}\enlargethispage{-4pt}

\noindent In Table~\ref{tabel:neumann} we list the number of iterations for
...
with the $\mathrm{CG}_1$ method.}\label{tabel:neumann}\vspace*{-5pt}
\end{table}
...
for convergence and estimates on the \nobreak condition number $\kappa$ with
respect to different discretization methods and different
characteristic cell sizes $h$. The problem we are solving is the
so-called lid driven cavity problem; that is, $f=0$ and \hbox{$g = (1,0)$} for
...
by\vspace*{2.5pt}
\[
\int_\Omega \Div u \, q  - \alpha h^2 \, \Grad p \cdot \Grad q \dx,\vspace*{2.5pt}
\]
...
\begin{tabular}{cccccccc}\toprule
  ...
  \caption{Estimated condition number $\kappa$ and number of
    ...
    methods $\mathrm{CG}_2-\mathrm{CG}_1$, $\mathrm{CG}_2-\mathrm{DG}_0$, and $\mathrm{CG}_1-\mathrm{CG}_1$-stab are stable, while
    $\mathrm{CG}_1-\mathrm{CG}_1$ is not.}\label{stokes:ex}\vspace*{6pt}
\end{table}
...
\]
Such a $\mathcal{B}$ can be defined as\vspace*{5pt}
\[
\mathcal{B}
=
...
where
\begin{align}
(K u, v) &= \int_\Omega u\cdot v  +  k \Grad u: \Grad v \dx, \\[3pt]
(L p, q) &= \int_\Omega k^{-1} p q \dx, \\[3pt]
...
\centering
\begin{tabular}{cccccccc}\toprule
    $k\backslash h$ & $2^{-4}$ & $2^{-5}$ & $2^{-6}$ & $2^{-7}$ & $2^{-8}$ \\\midrule
    1.0 & 13.6 & 13.6 & 13.6 & 13.7 & 13.7 \\
    0.1 & 13.4 & 13.5 & 13.6 & 13.6 & 13.6 \\
    0.01 & 12.8 & 13.2 & 13.4 & 13.5 & 13.6 \\
    0.001 & 11.0 & 12.3 & 13.0 & 12.3  & 13.5 \\\bottomrule
\end{tabular}
\caption{The convergence with respect to $k$ and mesh refinements for the time-dependent Stokes problem when using the $\mathrm{CG}_2-\mathrm{CG}_1$ method.}\label{timestokes:ex}
\end{table}
...
of the Hodge Laplacian,\vspace*{-3pt}
\begin{align}
...
\Div u -p &=  0 \quad \mbox{in} \ \Omega, \label{mixed:hodge2} \\
             ...
             p          &= 0 \quad \mbox{on} \ \partial \Omega.\vspace*{-3pt}
\end{align}
The variational form is: \\
Find $u, p \in H_0(\operatorname{curl}) \times H^1_0$ such that\vspace*{-3pt}
\begin{align}
\int_\Omega \nop{curl} u \cdot \nop{curl} v \dx
...
to invert the diagonal. We then obtain the following approximation of $L$:\vspace*{1.5pt}
\[
L  = A +  B^* (\mathrm{diag}(C))^{-1} B,\vspace*{1.5pt}
\]
...
\noindent Here, $D$ is a discrete Laplacian. The other preconditioner is
\[
\mathcal{B}_2  =
...
\begin{table}[!t]
\linenumbersep-10.4pc
\centering
\begin{tabular}{ccccccc}\toprule
    $h$ & $2^{-1}$ & $2^{-2}$ & $2^{-3}$ & $2^{-4}$  & $2^{-5}$ \\\midrule
  ...
  \caption{The estimated condition number $\kappa$ with respect to
    ...
    formulation of the Hodge Laplacian.}  \label{table:hodge}\vspace*{-5pt}
\end{table}
