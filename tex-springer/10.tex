...
\begin{figure}
\centering
\includegraphics{0001335343/272415_1_en_10_fig1_print.eps}
...
dashed lines indicate data flow.}\label{fig:logg-2:fenicsmap}\vspace*{5pt}
\end{figure}
...
\bwfig
  \centering
\includegraphics{0001335343/272415_1_en_10_fig2_print.eps}
...
F(x) = 0,\vspace*{-3pt}
\end{equation}
\looseness-1where $x \in \mathbb{R}^{n}$ and $F: \mathbb{R}^{n} \rightarrow
...
differential equation is shown below.\vspace*{-3pt}
%%
\begin{c++}
...
\noindent Meshes must be stored in the DOLFIN XML format. The following example
...
\begin{figure}[!t]
  ...
  \includegraphics{0001335343/272415_1_en_10_fig3_print.eps}
  \caption{Each entity of a mesh is identified by a pair $(d, i)$
    ...
    dimension~$d$.}\label{fig:logg-2:entities}\vspace*{7pt}
\end{figure}


\begin{table}[!t]
\linenumbersep-10.8pc
\centering
\begin{tabular}{lcc}\toprule
  ...
  \caption{Mesh entities and their dimensions/codimensions. The
    ...
    dimension and $d$ is the dimension.}\label{tab:logg-2:entities}\vspace*{-4pt}
\end{table}
...
\label{tab:logg-2:connectivity}\vspace*{4pt}
\end{table}
...
returned objects are therefore directly usable from within Python.\index{finite element!list of supported}

\begin{table}[!b]
\linenumbersep-10.5pc
\vspace*{10pt}
  \centering
  \begin{tabular}{ll}
    \toprule
...
throughout the entire toolchain.}\label{tab:logg-2:elements}
\end{table}
...
\bwfig
  ...
  \includegraphics{0001335343/272415_1_en_10_fig5_print.eps}
...
\emp{FunctionSpace}, the \emp{FunctionSpace} is shared \nobreak between the
\emp{Function}s.
...
the function $f(x, y) = \sin x \, \cos y$ as an \emp{Expression}:\vspace*{-2pt}
%%
\begin{c++}
...
differently in C++ and Python.\vspace*{-2pt}
%%
\begin{c++}
class MyExpression : public Expression
{
...
\noindent The above \emph{functor} construct for the definition of expressions
...
interface may, alternatively, use a simpler syntax:\vspace*{2pt}
%%
\begin{python}
...
\noindent The above code will automatically generate subclasses of the DOLFIN C++ \emp{Expression} class that overload the \emp{eval} function. This has the
...
string.  For example, one may use the variable \emp{t} to denote time:\vspace*{2pt}
%%
\begin{python}
...
forms for the discretization of a linear elastic problem:\vspace*{2pt}
%%
\begin{uflcode}
...
\noindent This should be compared to the corresponding mathematical notation:\vspace*{2pt}
%%
\begin{align} \label{eq:logg-2:elasticity1}
a(u, v) &= \int_{\Omega} \sigma(u) : \epsilon(v) \dx,\\[7pt]
L(v)    &= \int_{\Omega} f \cdot v \dx.\vspace*{2pt}
\end{align}
%%
...
\noindent This code may be compiled using a UFL/UFC compliant form compiler to
generate UFC C++ code. For example, using FFC:\vspace*{2pt}
%%
\begin{bash}
...
\noindent This generates a C++ header file (including implementation) named
...
form (\emp{a}), respectively:\vspace*{2pt}
%%
\begin{c++}
Vector b;
Matrix A;
...
memory architectures\break (clusters) using MPI and shared memory
...
displacement of an elastic body may be expressed as\vspace*{2pt}
%%
...
parts, we obtain\vspace*{2pt}
%%
\begin{equation}
  \int_{\Omega} \sigma(u) : \epsilon(v) \dx - \int_{\partial\Omega}
  (\sigma \cdot n) \cdot v \ds = \int_{\Omega} f \cdot v \dx,\vspace*{2pt}
\end{equation}
%%
...
$\Gamma_{\mathrm{N}}$ to obtain\vspace*{8pt}
%%
\begin{equation}
  \int_{\Omega} \sigma(u) : \epsilon(v) \dx
    = \int_{\Omega} f \cdot v \dx + \int_{\Gamma_{\mathrm{N}}} g \cdot v \ds.\vspace*{4pt}
\end{equation}
%%
...
or as part of a Python script:\vspace*{2pt}
%%
\begin{uflcode}
a = inner(sigma(u), sym(grad(v)))*dx
...
\end{python}
%%
Python users may also use the following compact syntax:\vspace*{2pt}
%%
\begin{python}
...
\noindent To speed up the application of Dirichlet boundary conditions, users of the Python interface may also use the function \emp{compile\_subdomains}. For
...
solved similarly in C++.\vspace*{-3.2pt}
%
\begin{python}
u  = Function(V)
v  = TestFunction(V)
...
is illustrated by the following code examples:\vspace*{-3.2pt}
%\pagebreak
%
\begin{python}
u = Function(V)
...
\emp{NonlinearVariationalSolver}, issue the following commands:\vspace*{-3.2pt}
%
\begin{c++}
...
From Python, one can also plot expressions and finite elements:\vspace*{2pt}
%%
\begin{python}
plot(grad(u))
plot(u*u)
...
\makeatletter
\def\img@cmode{\hskip-5pt\begin{turn}{90}\rlap{\kern42\p@\@img@cmode{\@cmodetext}}\end{turn}}
\makeatother

\begin{figure}[!t]
\bwfig
\centering
\includegraphics{0001335343/272415_1_en_10_fig6_print.tif}
\caption{Plotting a mesh using the DOLFIN \emp{plot} command, here
the mesh \emp{dolfin-1.xml.gz} distributed with DOLFIN.}
\label{fig:logg-2:plots,mesh}\vspace*{10pt}
\end{figure}
...
\bwfig
\centering
\includegraphics{0001335343/272415_1_en_10_fig7_print.tif}
    ...
    Figure~\ref{fig:logg-2:plots,mesh}.}\label{fig:logg-2:plots,function}
\end{figure}
...
extension, as illustrated in the following code examples:\vspace*{2pt}
%%
\begin{c++}
...
\noindent The standard PVD format is ASCII based, hence the file size can
...
PVD-based \emp{File} object:\vspace*{2pt}
%%
\begin{c++}
...
\noindent If multiple \emp{Function}s are written to the same file (by repeated
...
\emp{t} of each snapshot. This is done as illustrated below:\vspace*{2pt}
%%
\begin{c++}
File file("solution.pvd", "compressed");
...
\noindent Storing the time is particularly useful when animating simulations that use a varying time step.
...
illustrated in the following code examples:\vspace*{2pt}
%%
\begin{c++}
...
\end{python}
%%
\looseness-1Data in a \emp{TimeSeries} are stored in a binary format with one file for each stored dataset (\emp{Vector} or \emp{Mesh}) and a common
...
\bottomrule
\end{tabular}
\caption{Log levels in DOLFIN.}\label{tab:logg-2:loglevels}
\end{table}
...
code examples below.\vspace*{-3pt}
%%
\begin{c++}
...
command, as illustrated in the code examples below.\vspace*{-3pt}
%%
\begin{c++}
info(vector);
info(matrix);
...
\noindent In the Python interface, the same short informal message can be printed by calling \emp{print mesh}. To print more detailed data, one may
...
\noindent The detailed output for some objects may be very lengthy.
...
following code illustrates how to set some parameters for a Krylov solver:\enlargethispage{6pt}
%%
\begin{c++}
KrylovSolver solver;
...
\begin{figure}[!b]
\centering
\includegraphics{0001335343/272415_1_en_10_fig8_print.tif}
...
a color.}\label{fig:logg-2:parallel}\vspace*{6pt}
\end{figure}
...
\noindent The classes and functions in the \emp{cpp} module have the same
...
be intentionally excluded. An interested reader can examine particular classes or functions in the code for a full understanding of the implementation.
...
\emp{FunctionSpace} constructor takes a \emp{cpp.Mesh} (\emp{dolfin.Mesh}):\vspace*{2pt}
%%
\begin{python}
mesh = UnitSquare(8, 8)
V = FunctionSpace(mesh, "Lagrange", 1)
...
\noindent In the Python constructor of \emp{FunctionSpace}, a
...
a user to directly assemble a declared UFL form:\vspace*{2pt}
%\pagebreak
%%
\begin{python}
mesh = UnitSquare(8, 8)
...
\noindent The \emp{assemble} function is a thin wrapper layer around the wrapped
\emp{cpp.assemble} function. The following pseudo code illustrates what
happens in this layer:\vspace*{2pt}
%%
\begin{python}
...
\noindent Here, \emp{form} is a \emp{ufl.Form}, which is used to generate a
...
from a \emp{ufl.Form}:\vspace*{2pt}
%%
\begin{python}
class Form(cpp.Form):
...
\noindent The \emp{form} is first passed to the \emp{dolfin.jit} function,
...
compiler can be chosen by setting:\vspace*{2pt}
%%
\begin{python}
...
\noindent The default form compiler is \emp{"ffc"}. The \emp{jit} function of
the form compiler returns the JIT compiled \emp{ufc::form} together
with a \emp{ufl.FormData} object. \pagebreak The latter is a data structure
