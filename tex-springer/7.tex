...
is the tensor representation presented in \nobreak Chapter~\ref{chap:kirby-8}.
...
$\Omega$, which is denoted by~$\mathcal{T}$. \pagebreak If we let
...
coefficient $w$.  The code for evaluating \emp{F0} in \nobreak Figure~\ref{oelgaard-2:fig:standard_code} is an exact translation of
...
%%%}
%%%\end{c++}
\includegraphics{0001335340/272415_1_en_7_fig1_print.eps}
...
\label{oelgaard-2:fig:standard_code}\vspace*{6pt}
\end{figure}


\section{Quadrature optimizations}
\label{oelgaard-2:sec:quadrature_optimizations}
\index{quadrature representation!optimization of}
...
%%%}
%%%\end{c++}
\includegraphics{0001335340/272415_1_en_7_fig2_print.eps}
...
\label{oelgaard-2:fig:O_zeros_code}\vspace*{6pt}
\end{figure}
...
expression $x(y+z)+2xy$, which after expansion of the first
...
expressions, \pagebreak which take place before the simplification, will
...
%%%}
%%%\end{c++}
\includegraphics{0001335340/272415_1_en_7_fig3_print.eps}
\caption{Part of the generated code for the weighted Laplacian using
  linear elements in two dimensions with optimization option \emp{-f
...
%%%}
%%%\end{c++}
\includegraphics{0001335340/272415_1_en_7_fig4_print.eps}
\caption{Part of the generated code for the weighted Laplacian using
  linear elements in two dimensions with optimization option \emp{-f
...
command-line option \emp{-f precompute\_basis\_\break const} should
...
%%%%}
%%%%\end{c++}
\includegraphics{0001335340/272415_1_en_7_fig5_print.eps}
\caption{Part of the generated code for the weighted Laplacian using
  linear elements in two dimensions with optimization option \emp{-f
...
\ffc{} implements a few efficient quadrature schemes for integrating
...
\centering
  %\ffigbox{
\includegraphics{0001335340/272415_1_en_7_fig6_print.eps}
  \caption{Run-Time performance for the weighted Laplace
      equation for different compiler options.  The $x$-axis shows the
      \ffc{} compiler options, and the colors denote the g++ compiler
      options.}\label{oelgaard-2:fig:laplace_stats_2}\vspace*{5pt}%}
...
using the \emp{-O2} option, 5.23 when using the \emp{-O2
-funroll-loops} option and  7.75 when using the \emp{-O3} option.
The \emp{-O3} option does not appear to
improve the run-time noticeably beyond the improvement observed for the
\emp{-O2 -funroll-loops} option when the \ffc{} optimization option
\emp{-zeros} is used.  Using the \ffc{} optimization option
...
count compared to the standard \nobreak quadrature representation.  A curious
...
\emp{-O3}, in which case the run-time has improved by a
...
hyperelasticity example.}\label{oelgaard-2:tab:hyper_stats_1}\vspace*{-7pt}
\end{table}
...
\begin{figure}[!t]
  %\ffigbox{
\centering
\includegraphics{0001335340/272415_1_en_7_fig7_print.eps}
  \caption{Run-Time performance for the hyperelasticity
...
The test and trial functions are denoted by $v, u \in V_{h}$, with\vspace*{3pt}
\begin{equation}
V_{h} = \bracc{v \in [H^{1}\brac{\Omega}]^2: \ v\vert_T \in
[P_{q}\brac{T}]^2 \foralls T \in \mathcal{T}}
 \label{oelgaard-2:eq:elastictity_H1_vector_space}\vspace*{5pt}
\end{equation}
...
\citet{OelgaardWells2010}.}\label{oelgaard-2:tab:elasticity2D_complex_comparison}
\end{table}
