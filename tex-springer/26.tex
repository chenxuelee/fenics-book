...
of examples that demonstrate implementation approaches.\vadjust{\pagebreak} Finally,
...
%%
\begin{equation}
  \Pi_{\rm int} = \int_{\Omega_{0}} \Psi_{0}\brac{u} \dx,\vspace*{-3pt}
\end{equation}
%
where $\Psi_{0}$ is the stored strain energy density, and an
external potential energy functional of the form\vspace*{-3pt}
%%
\begin{equation}
  \Pi_{\rm ext} = - \int_{\Omega_{0}} b_{0} \cdot u \dx
      - \int_{\Gamma_{0, N}} h_{0} \cdot u \ds.\vspace*{-3pt}
\end{equation}
%
...
minimizes the potential energy:\vspace*{-3pt}
%%
\begin{equation}
  \min_{u \in V} \Pi,\vspace*{-3pt}
\end{equation}
%%
...
to solving~\eqref{eq:oelgaard-1:gov_generic} with\vspace*{-3pt}
%%
\begin{align}
\label{eq:oelgaard-1:gov_energy_variation}
  F(u; w) := D_{w} \Pi\brac{u}
      = \left. \frac{d \Pi\brac{u + \epsilon w}}{d \epsilon} \right|_{\epsilon = 0}.\vspace*{-3pt}
\end{align}
%%
...
tensor for an isotropic, homogeneous material is given by\vspace*{-3pt}
%
\begin{align}
  \sigma = 2\mu \varepsilon + \lambda {\rm tr}(\varepsilon) I,
\label{eq:oelgaard-1:elas_stress_def}\vspace*{-3pt}
\end{align}
%
...
the stress and the strain can also be expressed~as\vspace*{-3pt}
%
\begin{align}
  \sigma = \mathcal{C} : \varepsilon,\vspace*{-3pt}
\end{align}
%%
where\vspace*{-3pt}
%%
...
C &= F^{\rm T} F,
\\[2pt]
E &= \frac{1}{2} \brac{C - I},\vspace*{3pt}
\end{align}
%
...
expression for the strain energy density function:\vspace*{3pt}
%
\begin{equation}
\label{eq:oelgaard-1:hyp_st_venant_kirchhoff}
  \Psi_{0} = \frac{\lambda}{2} \brac{{\rm tr} \, E}^{2} + \mu E:E,\vspace*{3pt}
\end{equation}
%
...
of a hyperelastic model is the compressible neo-Hookean model:\vspace*{3pt}
%
\begin{equation}
\label{eq:oelgaard-1:hyp_neo_hookean}
  \Psi_{0} = \frac{\mu}{2} (I_{C} -3) - \mu \ln J + \frac{\lambda}{2} (\ln J)^{2},\vspace*{3pt}
\end{equation}
%
...
using two Gauss quadrature points on an element $T \in \mathcal{T}_h$:\vspace*{-2pt}
%
\begin{equation}
\label{eq:oelgaard-1:lin_issue_f}
f_{T,i_{1}} := \sum_{q=1}^2 \sum_{\alpha=1}^2 \psi^{T}_{\alpha}\brac{x_{q}} \sigma_{\alpha}
\phi^{T}_{i_{1},x}\brac{x_q} W_{q},\vspace*{-2pt}
\end{equation}
%
...
\eqref{eq:oelgaard-1:lin_issue_f} is:\vspace*{-2pt}
%
\begin{align}
\label{eq:oelgaard-1:lin_issue_linearised_stiffness}
A^{\star}_{T,i} := \frac{df_{T,i_{1}}}{du_{i_{2}}},\vspace*{-2pt}
\end{align}
%
...
\frac{ d \sigma_{\alpha}}{du_{i_{2}}}
    ...
    = D_{\alpha} \frac{d \varepsilon_{\alpha}}{du_{i_{2}}},\vspace*{-1pt}
\end{equation}
%
...
derivative of the displacement field is evaluated at~$x_{\alpha}$:\vspace*{-1pt}
%
\begin{equation}
\label{eq:oelgaard-1:lin_issue_strain}
\varepsilon_{\alpha} = \sum_{i_{2}=1}^3 \phi^{T}_{i_{2},x}\brac{x_{\alpha}}u_{i_{2}}.\vspace*{-1pt}
\end{equation}
%
Inserting~\eqref{eq:oelgaard-1:lin_issue_stress}
and~\eqref{eq:oelgaard-1:lin_issue_strain}
into~\eqref{eq:oelgaard-1:lin_issue_linearised_stiffness} yields:\vspace*{-1pt}
%
\begin{equation}
\label{eq:oelgaard-1:lin_issue_k_star}
A^{\star}_{T,i} = \sum_{q=1}^{2}\sum_{\alpha=1}^2 \psi^{T}_{\alpha}(x_{q}) D_{\alpha}
\phi^{T}_{i_{2},x}(x_{\alpha}) \phi^{T}_{i_{1},x}(x_{q}) W_{q}.\vspace*{-1pt}
\end{equation}
%
...
which leads to the bilinear form:\vspace*{-1pt}
%
\begin{equation}
a(u, w) = \int_{\Omega} D u,_{x} w,_{x} \dx,\vspace*{-1pt}
\end{equation}
%
...
%%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig1_print.eps}
...
\label{fig:oelgaard-1:plas_nonlinear_poisson_code}\vspace*{6pt}
\end{figure}
...
%%%solver.solve()
%%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig2_print.eps}
...
%%%};
%%%\end{c++}
\includegraphics{0001335359/272415_1_en_26_fig3_print.eps}
...
\label{fig:oelgaard-1:plasticity_interface}\vspace*{12pt}
\end{figure}
...
%%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig4_print.eps}
...
%%%}
%%%\end{c++}
\includegraphics{0001335359/272415_1_en_26_fig5_print.eps}
\caption{Code extract for solving a plasticity problem.}
\label{fig:oelgaard-1:plas_cpp_code}\vspace*{6pt}
\end{figure}
...
%%%file << u;
%%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig6_print.eps}
\caption{Complete DOLFIN solver for the compressible neo-Hookean model,
formulated as a minimization problem.}
\label{fig:oelgaard-1:hyperelastic_solver_code}\vspace*{-3pt}
\end{figure}
...
%%h = Traction(T/4.0)
%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig7_print.eps}
...
\label{fig:oelgaard-1:time_ufl_code}\vspace*{6pt}
\end{figure}
...
%%%    file << u
%%%\end{python}
\includegraphics{0001335359/272415_1_en_26_fig8_print.eps}
\caption{Continuation of Python code extract for solving for a dynamic
problem.}\label{fig:oelgaard-1:time_ufl_code2}\vspace*{3pt}
\end{figure}
