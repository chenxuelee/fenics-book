...
\index{transition to turbulence}
...
distributed CBC.PDESys package, see also \citet{Mortensen2011}.
...
largest and most energetic turbulence \pagebreak structures are primarily responsible
for \nobreak efficient mixing of momentum and other scalar quantities. These
...
whereas odd derivatives lead to\break dispersion.
...
$\Gamma$. The $L^2$ inner product of fields on $\Omega$ is denoted as\vspace*{1.5pt}
\begin{equation}
 \inner{\vec{a}}{\vec{u}} = \int_{\Omega} \vec{a}\cdot \vec{u} \dx,
 \label{eq:mortensen:L2}\vspace*{1.5pt}
\end{equation}
...
convective term,\vspace*{1.5pt}
\begin{equation}
B(\vec{u},\vec{a}) =  \vec{u} \cdot \nabla \vec{a}\vspace*{1.5pt}
\end{equation}
can be multiplied by the vector $\vec{b}$ and integrated by parts to yield\vspace*{1.5pt}
\begin{equation}
...
\label{eq:mortensen:Bu1}\vspace*{1.5pt}
\end{equation}
...
where\vspace*{5pt}
\begin{equation}
\vec{u}_{n-\alpha}=(1-\alpha) \vec{u}_{n} + \alpha \vec{u}_{n-1},\vspace*{1.5pt}
\end{equation}
...
\end{align}
The\enlargethispage{12pt} explicit Adams-Bashforth
...
\item In a fully coupled formulation, we can assemble small
  ...
  identical to the vector space \emp{V.sub(0)}:\vspace*{-2pt}
\begin{python}
uc = TrialFunction(Vc)
vc = TestFunction(Vc)
...
the beginning of Section~\ref{sec:mortensen:NS-solver} for the
...
\caption{CPU times (total/inside time loop) for the fully coupled and
  ...
  timings for the solver and assembler respectively.}\label{tab:mortensen:speed-up}\vspace*{-8pt}
\end{table}
...
Burgers' equation reads:\vspace*{-3pt}
\begin{align}
...
\includegraphics{0001335355/272415_1_en_22_fig1_print.eps}
  ...
  \label{fig:mortensen:burgers_KE}\vspace*{5pt}
\end{figure}

\vspace*{3pt}
\subsection{Orr--Sommerfeld equation}
\label{sec:mortensen:OS}
\index{Orr--Sommerfeld equation}
...
dimensions). Consequently the velocity perturbations are\vspace*{3pt}
\begin{align}
 u'(x,y,t)&=\frac{\partial \psi}{\partial y}=i\alpha \phi \, \mathrm{exp}(i(\alpha x- \lambda t)),\\[6pt]
 v'(x,y,t)&=-\frac{\partial \psi}{\partial x}=-\phi' \, \mathrm{exp}(i(\alpha x- \lambda t)).\vspace*{3pt}
\end{align}
...
\label{eq:mortensen:OrrS}\vspace*{3pt}
\end{equation}
...
%\bwfig
\centering
\includegraphics{0001335355/272415_1_en_22_fig2_print.eps}
...
\begin{equation}
  E(t)= \int_0^{2\pi}\int_{-1}^{1} \left( \left[u-(1-y^2)\right]^2
    + v^2 \right) \dx.
\end{equation}
The exact analytical perturbation energy at any time should be\vspace*{-3pt}
\begin{equation}
 \frac{E(t)}{E(0)}=\mathrm{exp}(i \mathrm{Imag}(\lambda) t).\vspace*{-3pt}
\end{equation}
...
viscosity, etc. The mesh and some necessary parameters are declared as:\vspace*{-2pt}
\begin{python}
from dolfin import *
...
resides within the CBC.PDESys package. For the initialization of DOLFIN \emp{Functions}
...
procedure for the fractional step solver is:\vspace*{-2pt}
\begin{python}
...
\begin{figure}[!t]
\centering
\includegraphics{0001335355/272415_1_en_22_fig3_print.eps}
...
forms, here represented with solid, dashed and dotted lines
    ...
    approximately 2\% less throughout).}\label{fig:mortensen:OS_init_cfl}\vspace*{7pt}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics{0001335355/272415_1_en_22_fig4_print.eps}
...
\vspace*{-4pt}\enlargethispage{6pt}
\paragraph{Results}
...
error in the perturbation flow energy computed as\vspace*{-3pt}
\begin{equation}
\text{Error} = \sum_{k=0}^N \frac{|E(t_k)-\mathrm{exp}(i \mathrm{Imag}(\lambda) t_k)|}{N}.
...
\begin{figure}[!b]\vspace*{-4pt}
\centering
\includegraphics{0001335355/272415_1_en_22_fig5_print.eps}
\caption{Temporal evolution of the perturbation energy. The gray and
    ...
    The left and right figures on the lower row use implicit convection \eqref{eq:mortensen:IM2} and ${\rm CFL} = 0.1$ and 0.05 respectively. Note that for explicit convection and the standard implicit form the gray and black curves are practically
    identical.}\label{fig:mortensen:OS_long_time}\vspace*{-10pt}
\end{figure}
...
with explicit convection. The larger \nobreak splitting \nobreak errors obtained with the
...
\includegraphics{0001335355/272415_1_en_22_fig6_print.eps}
...
the divergence-free, end-of-step, velocity field. For the long integration times often associated with \nobreak turbulence applications, though, we find that the implicit form remains stable and accurate where the explicit form cannot maintain sufficient stability.
