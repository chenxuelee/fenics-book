...
we may consider the mapping\vspace*{6pt}
\begin{equation}
  L_u \equiv F(u; \cdot) : \hat{V} \rightarrow \R,\vspace*{2pt}
\end{equation}
...
product of a sequence of argument spaces and coefficient spaces:\vspace*{-1.5pt}
\begin{equation}
  \begin{split}
...
\kappa(x)$,\vspace*{-1.5pt}\enlargethispage{6pt}
\begin{equation}
-\Div(\kappa \Grad u)=f.\vspace*{-1.5pt}
\end{equation}
...
pressure~$p$ of an incompressible fluid read:\vspace*{-1.5pt}
\begin{equation}
  \begin{split}
...
the nonlinear term $\Grad u \cdot u$ against a test function $v$:\vspace*{-1.5pt}
\begin{equation}
  a(u; v) = \int_{\Omega} (\Grad u \cdot u) \cdot v \dx.\vspace*{-1.5pt}
\end{equation}
This is a linear form ($\rho = 1$) with one coefficient ($n = 1$). We
may linearize around a fixed velocity~$\bar{u}$ to obtain\vspace*{-0.5pt}
\begin{equation}
  a(u; v) = a(\bar{u}; v) + a'(\bar{u}; v) \delta u + \mathcal{O}(\delta u^2),\vspace*{-0.5pt}
\end{equation}
where $u = \bar{u} + \delta u$. The linearized operator $a'$ is here
given by\vspace*{-1.5pt}
\begin{equation}
  a'(\bar{u}; \delta u, v) \equiv a'(v; \bar{u}) \delta u =
  \int_{\Omega}
  (\Grad \delta u \cdot \bar{u}) \cdot v +
  (\Grad \bar{u} \cdot \delta u) \cdot v \dx.\vspace*{-1.5pt}
\end{equation}
This is a bilinear form ($\rho = 2$) with one coefficient ($n = 1)$.
We may also consider the \emph{trilinear} form\vspace*{-1.5pt}
\begin{equation}
a(w, u, v) = \int_{\Omega} (\Grad u \cdot w) \cdot v \dx.\vspace*{-1.5pt}
\end{equation}
...
generated on the object (for a flow in the $x$-direction):\vspace*{-1.5pt}
\begin{equation}
  \begin{split}
...
\includegraphics{0001335338/272415_1_en_5_fig1_print.eps}
...
that may be expressed as the following canonical form:\vspace*{-2.5pt}
...
\end{equation}
Here,\AQ{Please provide text citation for Fig.\,5.1.} each $\Omega_k$ denotes a union of mesh cells covering a subset
...
\begin{equation}
\begin{split}
A_{ij} &= a(\phi_j, \hat{\phi}_i), \quad i,j = 1,2,\ldots,N, \\[3pt]
...
local cell of the mesh. \pagebreak In particular, consider the discretization of
...
2, \ldots, \rho$, we obtain the cell tensor\vspace*{2.5pt}
\begin{equation} \label{eq:kirby-5:celltensor}
A_{T,i}= a_T(w_1, w_2, \ldots, w_n;
...
%\bwfig
\centering
\includegraphics{0001335338/272415_1_en_5_fig2_print.eps}
...
\caption{The cell tensor~$A_T$, exterior facet tensor $A_S$, and
    ...
    discretizes the multilinear form.}\label{fig:kirby-5:tensors}
%\vspace*{8pt}
\end{figure}
...
Consider the bilinear form\vspace*{9pt}
\begin{equation}
...
as above by%\vspace*{-1.5pt}
\begin{equation}
  A_{ij} = a(\phi^2_j, \phi^1_i).
%\vspace*{-1.5pt}
\end{equation}
For any given vector $U \in \R^N$, it follows that%\vspace*{-1.5pt}
\begin{equation}
  (A U)_i = \sum_{j=1}^N A_{ij} U_j
  = \sum_{j=1}^N a(\phi^2_j, \phi^1_i) U_j
  = a\left(\sum_{j=1}^N U_j \phi^2_j, \phi^1_i \right)
  = a(u_h, \phi^1_i) = (\mathcal{A} a) (u_h; \phi^1_i),%\vspace*{-1.5pt}
\end{equation}
...
corresponding finite element approximation:%\vspace*{-1.5pt}
\begin{equation}
  (A U)_i = (\mathcal{A} a) (u_h; \phi^1_i).%\vspace*{-1.5pt}
\end{equation}
...
form language by calling the \emp{action} function:\vspace*{-2pt}
\begin{python}
...
problem to find $u \in V$ such that\vspace*{3pt}
...
\end{equation}
or simply\vspace*{7pt}
\begin{equation}
a'^*(z, v) = \mathcal{M}' (v) \quad \foralls v \in V,\vspace*{1pt}
\end{equation}
