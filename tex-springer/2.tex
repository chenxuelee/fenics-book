...
on the boundary $\partial \Omega$ of~$\Omega$:\index{boundary condition!Dirichlet}\index{boundary condition!Neumann}\vspace*{1.5pt}
...
subject to a \pagebreak heat source $f$. Letting $\sigma = \sigma(x)$ denote heat flux, it follows by conservation of energy that the outflow of energy
over the boundary\AQ{Please provide citation for Fig.\,2.1.} $\partial\omega$ of any test volume
...
\bwfig
\centering
\includegraphics{0001335335/272415_1_en_2_fig1_print.eps}
\caption{Poisson's equation is a simple consequence of balance of
energy in an arbitrary test volume~$\omega \subset \Omega$.}\vspace*{-2pt}
\end{figure}
...
$\tau$. Summing up and integrating by parts, we find that\vspace*{6pt}
\begin{equation}
...
\begin{equation} \label{eq:system,nonlinear}
  b(U) = 0,
\end{equation}
where $b : \R^N \rightarrow \R^N$ and\vspace*{7pt}
\begin{equation}
...
\end{equation}
where $\delta U^k$ solves the linear system\vspace*{5pt}
...
As an example, consider the following nonlinear Poisson \nobreak equation:
...
\bwfig
\centering
\includegraphics{0001335335/272415_1_en_2_fig2_print.eps}
  %\ffigbox{
...
\bwfig
  ...
  \includegraphics{0001335335/272415_1_en_2_fig3_print.eps}
...
\bwfig
  ...
  \includegraphics{0001335335/272415_1_en_2_fig4_print.eps}
  \caption{Local-to-global mapping for a simple mesh consisting of
    ...
    \ldots, 6$.}\label{fig:dofmap}\vspace*{6pt}
\end{figure}
...
\begin{figure}
  ...
  \includegraphics{0001335335/272415_1_en_2_fig5_print.eps}
...
\includegraphics{0001335335/272415_1_en_2_fig6_print.eps}
\caption{The (affine) map $F_T$ from a reference cell $\hat{T}$
to a cell $T \in \mesh$.}\label{fig:kirby-7:affinemap}\vspace*{-4pt}
\end{figure}
...
which transform the linear system\vspace*{-2pt}
\begin{equation}
AU = b\vspace*{-2pt}
\end{equation}
into\vspace*{5pt}
...
\includegraphics{0001335335/272415_1_en_2_fig7_print.eps}
...
norm defined by the bilinear form~$a$). This follows by the Galerkin orthogonality $\inner{u - u_h}{v}_a \equiv a(u - u_h, v) = 0$ for all $v \in V_h$.}\label{fig:ceaslemma}\vspace*{-1.5pt}
\end{figure}
...
equivalent to the $V'$-norm of the\break residual $r$. To see this, note
that by the continuity of the bilinear form~$a$, we have
\begin{equation}
r(v)= L(v) - a(u_h, v) = a(u, v) - a(u_h, v) = a(u - u_h, v)
...
= L(u - u_h) - a(u_h, u - u_h) = r(u - u_h).\vspace*{5pt}
\end{equation}
It follows that\vspace*{2pt}
...
\hat{V}^* \rightarrow \R$ is defined by\vspace*{-3pt}
\begin{equation}
  a^*(v, w) = a(w, v) \quad \foralls (v, w) \in V^* \times \hat{V}^*.\vspace*{-3pt}
\end{equation}
The dual trial and test spaces are given by\vspace*{-3pt}
\begin{equation}
\begin{split}
V^* &= \hat{V},\\
...
problem leads us to the following representation of the error:\vspace*{-3pt}
\begin{equation}
  \begin{split}
...
dual solution:\vspace*{-3pt}
\begin{equation}\label{eq:aposteriori,dual}
\mathcal{M}(u) - \mathcal{M}(u_h) = r(z).\vspace*{-7pt}
\end{equation}
...
$\eta_T^2 = h_T^2 \|R\|_{T}^2 + h_T \|[\partial_n u_h]\|_{\partial T}^2$, we obtain the estimate\vspace*{-3pt}
\begin{equation}
