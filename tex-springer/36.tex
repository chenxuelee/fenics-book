...
supporting \pagebreak the following components: a suitable range of different
...
\pi^2 \sin(\pi x)\break \sin(\pi y)$. Consider the following mixed
...
see~Figure~\ref{rognes:fig:example}(a), and hence
it is a poor approximation to the correct solution.
%%\begin{figure}
%%  \centering
%%  {\label{rognes:fig:cg} \includegraphics[width=\twofigs]{rognes/CG}}
%%  {\label{rognes:fig:rt} \includegraphics[width=\twofigs]{rognes/RT}}
...
\bwfig
  \centering
  \includegraphics{0001335369/272415_1_en_36_fig1_print.eps}
  %\subfloat[Bad approximation]{\label{rognes:fig:cg} \includegraphics[width=\twofigs]{chapters/rognes/pdf/CG.pdf}}
  %\subfloat[Good approximation]{\label{rognes:fig:rt} \includegraphics[width=\twofigs]{chapters/rognes/pdf/RT.pdf}}
  \caption{The scalar variable approximation $u_h$ for two choices
    of mixed finite element spaces for the mixed Laplacian. The data
    are as defined immediately above~\eqref{rognes:eq:mixedlaplace}.
    The element spaces are $\rognesvectorcg{1}{2} \times \rognesdg{0}$
    in~(a) and $\rognesrt{1} \times \rognesdg{0}$
    in~(b). (The scales are less relevant for the
    current purpose and have therefore been
    omitted.)}
  \label{rognes:fig:example}
\end{figure}
...
Figure~\ref{rognes:fig:example}(b). This
...
where\vspace*{6pt}
\begin{equation}
Z_h = \{ v \in V_h \, | \, b(v, q) = 0 \quad \foralls q \in Q_h\}.\vspace*{1.5pt}
\end{equation}
...
\times Q_h$ and $\lambda \in \R$ satisfying\vspace*{2pt}
\begin{equation}
\label{rognes:eq:eigenvalue:infsup}
\langle u_h, v \rangle_{V} + b(v, p_h) + b(u_h, q) = - \lambda
\langle p_h, q \rangle_{Q}\vspace*{2pt}
\end{equation}
...
inspected as follows:\vspace*{1.5pt}
\begin{python}
print result
...
\noindent The following output appears:\vspace*{1.5pt}
\begin{progoutput}
...
\noindent \rognesascot{} characterizes this discretization family as
...
see~Figure~\ref{rognes:fig:example}(a).
...
result observed in
Figure~\ref{rognes:fig:example}(b) is thus
