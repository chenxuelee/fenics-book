...
\item
  ...
  up-to-date. \emph{All code presented in the book should be
    compatible with DOLFIN 0.9.9 (or later)!} Only minor interface changes (if
...
\item
  ...
  the chapter [kirby-6] as a template for line widths and colors.
\item
...
\item
  Use two dashes to typeset names such as Navier--Stokes,
  Runge--Kutta: \verb|Navier--Stokes|, \verb|Runge--Kutta|.
\item
...
\begin{itemize}
\item
  The order of test and trial functions has changed. The canonical
  variational problem is now $a(u, v) = L(v)$. Generally, we write:
  \begin{equation}
    F(f_1, \dots, f_m; v_1, \dots, v_n)
  \end{equation}
  for a form $F$ that is (possibly) nonlinear in the variables $f_1, \dots, f_m$
  and linear in the variables $v_1, \dots, v_n$.
\item
...
\begin{longtable}{lcl}
  ...
  $\| \cdot \|_{\Omega}$ & -- & if $\Omega$ is a domain: $L^2(\Omega)$ norm \\
...
\begin{longtable}{ccl}
  ...
  $\mathcal{L}$ &--&
  the degrees of freedom (linear functionals) on $\mathcal{V}$ \\
  %$N$ &--&
  ...
  $\ell_i$ &--&
  a degree of freedom (linear functional) on $\mathcal{V}$ \\
  %$\ell^0_i$ &--&
  %a degree of freedom (linear functional) on $\mathcal{V}_0$ \\
  %$\mathcal{P}_T$ &--&
  ...
  $\mathcal{V}$ &--&
  the finite dimensional function space in the Ciarlet finite element definition \\
  $P_q$ &--&
  ...
  \\
  $V'$ &--&
  the dual space (as in the bounded linear functionals) of a space $V$ \\
  $V_h$ &--&
