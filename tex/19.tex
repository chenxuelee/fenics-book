...
files included in \emp{dolfin.i}.

In \emp{typemaps.i} are typemaps and so called SWIG fragments, defined
...
\end{swigcode}
Here we specify that a function returning an \emp{unsigned int} will
use the SWIG provided type conversion macro: \emp{SWIG\_From(unsigned
...
\end{swigcode}
%$ to fool emacs highlight...
Here we rely on another fragment provided function:
\emp{PyInteger\_Check}, which we provide as a substitute to the built
in \emp{PyInt\_Check} function. The reason for that is that
...
function
\emp{convert\_numpy\_to\_}\-\emp{array\_no\_check(TYPE\_NAME)}, which
...
\end{swigcode}
Here we first ignore the C++ versions of the \emp{coordinates} and
...
following SWIG directives in the \emp{mesh\_pre.i} files.
\begin{swigcode}
