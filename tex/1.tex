...
executable Python source code files in a directory tree. File paths
...
which here is meant to indentify a 2D Poisson problem and help to
...
dimension (\emp{aD}), i.e., 1D, 2D, or 3D. We will occasionally only
...
functions $u_0$ and $f$. For our first implementation we must decide on specific
choices of $\Omega$, $u_0$, and $f$.  It will be wise to construct a
...
be frequently constructed throughout this tutorial.
...
given choices of $u_0$, $f$, and $\Omega$ may look as follows:
%%
\begin{python}
from dolfin import *
...
\index{finite element specifications}\index{CG finite element
  family}\index{Lagrange finite element family} The type of element is
...
\end{python}
The absolute and relative tolerances enter
...
the various parts of the matrix assemly and solve process.
...
\emp{.fenics/dolfin\_parameters.xml} in the user's home directory is
...
\emp{numpy} array (i.e., a copy), manipulate it, and insert
...
\right)}.
\end{equation}
For notational convenience we have dropped new symbols for the scaled
coordinates in \eqref{langtangen:poisson1:membrane:scaled:eq:rhs}.
Now $D$ is related $w$ through $D = AR^2w/(8\pi\sigma T)$.

Let us list the modifications of the
\emp{d1\_p2D.py} program that are needed code to solve this membrane problem:
\begin{enumerate}
  \item Initialize $T$, $A$, $R$, $x_0$, $y_0$, and $\sigma$ in the
...
with indices 0, 1, and 3, and with the name \emp{x}. Otherwise
...
look at a special case where it is easy to find an analytical solution
to compare with.
In the limit $\sigma\rightarrow\infty$, the function $f$
approaches the constant 4, which allows us to easily integrate the
equation (in radial coordinates) and obtain an exact solution
$w_{\rm e}(x,y) = 1-x^2-y^2$.  Practical values for an infinite $\sigma$
is 50 or larger, so we may insert a code segment that that
computes the difference $w-w_{\rm e}$
when $\sigma \geqslant 50$.
...
comparison of the numerical and (approximate) analytical solutions
(Section~\ref{langtangen:poisson1:verify1}).
\begin{python}
from dolfin import *
...
\end{python}
Here, \emp{u\_exact} will here be interpolated onto the function
...
\end{python}
Although \emp{nabla\_grad(u)} and \emp{grad(u)} are interchangable
...
visualization and data anlysis tools for \emph{structured
...
and easier than doing the same with data on unstructred meshes, and
...
The visualizations constructions shown above and used to generate the
...
\begin{figure}
  \centering
  \subfloat[]{\includegraphics[width=\twofigs]{chapters/langtangen/pdf/Poisson2D_Dvc_contour1.pdf}}
  \subfloat[]{\includegraphics[width=\twofigs]{chapters/langtangen/pdf/Poisson2D_Dvc_flux_x.pdf}}
...
\begin{figure}
  \centering
  \subfloat[]{\includegraphics[width=\twofigs]{chapters/langtangen/pdf/Poisson2D_Dvc_surf1.pdf}}
  \subfloat[]{\includegraphics[width=\twofigs]{chapters/langtangen/pdf/Poisson2D_Dvc_mesh1.pdf}}
  \caption{Examples on plots created by transforming the finite
...
solve(a == L, u, bcs)
# or
problem = LinearVariationalProblem(a, L, bcs)
\end{python}
...
Instead of \emp{solve(a == L, u, b)} we now write\index{assemble}\index{linear systems
...
The conditions are also incorporated in symmetric way to preserve
...
A complete code where linear system $AU=b$ is explicitly assembled and
...
the difference between \emp{assemble} and \emp{assemble\_linear}
system.  The reader is encouraged to run the code for a $2\times 1$
...
solve(a == L, u, bcs)
\end{python}
The complete code is found in \emp{paD.py} (Poisson problem in anyD).
...
\index{derivative}\index{automatic differentiation}
...
J  = derivative(F, u_, du)  # Gautaux derivative in dir. of du
\end{python}
or
\begin{python}
u  = TrialFunction(V)
...
J  = derivative(F, u_, u)   # Gautaux derivative in dir. of u
\end{python}
...
\index{NonlinearVariationalSolver}\vspace*{3pt}
\begin{python}
...
\emp{dn2\_p2D.py}
program in the directory \emp{stationary/poisson}.  The new aspects
are
\begin{enumerate}
