...
in the file \emp{Poisson2D\_D2\break .py} remains almost the same.  Only two
...
found in in the file\break \emp{Poisson2D\_Dvc.py}. You can run it and confirm
...
the file\break \emp{nlPoisson\_pdeNewton.py} (for the fun of it we use
...
\end{python}
\vspace*{12pt}
\noindent where \emp{u} is a \emp{Function} representing the most recent solution.
...
\vspace*{12pt}\noindent The differentiation is done symbolically so no numerical approximation
...
\index{nonlinear variational problems}\vspace*{3pt}
\begin{python}
...
depending on whether the matrix is symmetric or not):\vspace*{-6pt}
\begin{python}
...
maximum allowable number of iterations, the\break
\emp{"gmres\_restart"} parameter tells how many
iterations GMRES performs before it restarts,\break
...
subdomain values. That is,\break \emp{subdomain.values()[i]} is the subdomain
