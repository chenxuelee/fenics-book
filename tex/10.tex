...
sub_domains = mesh.data().mesh_function("sub_domains")
\end{python}
To list data that associated with a given \emp{Mesh}, issue the
...
automatically by the class\break \emp{Variational\-Problem}, which will be
...
available parameters for \emp{LinearVariationalSolver} or
...
level to the \emp{info} and \emp{log} commands, as illustrated in the
code examples below.
%%
\begin{c++}
...
directory \emp{.fenics} in the user's home directory (in that order)
for a file named \emp{dolfin\_parameters.xml}. If found, these
