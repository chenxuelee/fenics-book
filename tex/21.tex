...
U = 0.5*(u0 + u)
F1 = (1/k)*inner(u - u0, v)*dx \
   + inner(inner(u0, nabla_grad(u0)), v)*dx \
...
\begin{equation}
  SD_{\delta}
  = \renni{\nabla v \,  u_h^{n-\frac{1}{2}}}{\delta_1 u_h^{n-\frac{1}{2}} \cdot \nabla u_h^{n-\frac{1}{2}}}
...
\rightarrow v + \delta_1 (\bar{u}^{n} \cdot \nabla v + \nabla q)$ and
...
Fv = (1/k)*inner(u - u0, v)*dx \
   ...
   + inner(pbar*n, v)*ds \
   - inner(f, v)*dx \
   + d1*inner(dot(W, nabla_grad(U)), dot(W, nabla_grad(v)))*dx \
