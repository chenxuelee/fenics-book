...
= \theta \sigma_f + (1 - \theta) \sigma_s$. We can also decompose the
stress into a pressure $pI$ and deviatoric part: $\sigma = \sigma^D - pI$.
...
In Unicorn, we have currently implemented two constitutive laws:
Newtonian fluid and incompressible Neo-Hookean solid \citep{HoffmanJanssonStockli2011}.
...
\begin{equation} \label{eq:TotalModel}
  \begin{split}
    R_u(W) &= \rho\left(\frac{\partial U_i}{\partial t} + U_j \frac{\partial U_i}{\partial x_j} \right) - \frac{\partial}{\partial x_j} \Sigma^D_{ij} + P - f_i,\\
...
\begin{equation}
  \begin{split}\label{eq:ALE}
    R_u(W) &= \rho\left(\frac{\partial U_i}{\partial t} + (U_j - \beta^h_j) \frac{\partial U_i}{\partial x_j} \right) - \frac{\partial}{\partial x_j} \Sigma^D_{ij} + P - f_i,
    \\
    R_p(W) &= \frac{\partial U_j}{\partial x_j}.
  \end{split}
\end{equation}
...
\begin{align}
  ...
  \inner{\Sigma^D_{ij} - P}{\frac{\partial}{\partial x_j} v^u_i} + SD^u(W, v^u) = 0, \\
...
well. We refer to \citet{Nazarov2009,Nazarov2011} for implementation details of
