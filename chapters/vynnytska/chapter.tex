...
a numerical algorithm for the simulation of these equations and an
...
k_{ch}/k_{th}$, see Table~\ref{vynnytska:table:variables}.
...
(still) the reference viscosity, $x_2$ is the second spatial
coordinate, and $b$ and $c$ are given additional parameters.
...
apply no flux conditions on the remainder of the boundary. This
condition can be viewed as a consequence of the no outflow conditions
for the velocity.

The initial temperature field is given by $T_0$.
In~\eqref{vynnytska:eq:ic1} below, we give an analytical expression
for this field based on boundary layer theory~\citep{KekenEtAl1997},
taking the value of $Ra$ with input from the upper $T_u$, lower $T_l$,
right $T_r$ and left $T_s$ parts of the domain into account:
\begin{subequations}
...
between $0$ and $1$, we performed an additional correction. According
...
below $0$ to $0$ and above $1$ to $1$. The initial composition $\phi$
is a step function equal to one on the bottom layer and to zero on the
top layer.
...
$Q_h$ be finite dimensional spaces, defined relative to the partition
...
property fields from numerical diffusion and dispersion errors, and is
motivated and described in detail by~\citet{LenardicKaula1993}.
...
specified in the detailed Algorithm~\ref{vynnytska:alg:algorithm}
below.
\begin{algorithm}
  \begin{tabbing}
...
In this section, we discuss the main structure of a DOLFIN
implementation of the numerical scheme described above in
...
while (t <= finish):
    ...
    \caption{Abridged code for the main predictor-corrector algorithm,
      ...
      solutions at the current time; \code{T\_}, \code{u\_},
      and \code{phi\_} for fields at the previous time;
      and \code{T\_pr}, \code{u\_pr}, and \code{phi\_pr} for predictor
      fields. These \code{Functions} are all initialized (but the
...
\item
  ...
  the previous and current time. Analogously, the Stokes equations for
  the predicted and corrected velocity depend on the predicted and
  corrected composition and temperatures through the viscosity and the
  source terms. The linear systems of equations therefore have to be
  assembled (in addition to solved) at each time. For simplicity, we
  therefore generate the variational forms describing the equations
  and define a new \code{VariationalProblem} for each of these
  problems at each time. The compute time used for this is
  insignificant in comparison with the time required for the solution
...
\item
  ...
  the \code{numpy} array of degrees of freedom. Since the time step
  hence varies in each iteration and with the mesh size, it is
  convenient to use a \code{Constant} for its representation in order
...
see~\eqref{vynnytska:eq:mixed}. In particular, as the rheology
...
def stokes(mesh, eta, f):
  ...
  \caption{Definition of variational forms for the Stokes equations
    ...
    extracted using the UFL functions \code{lhs}
    and \code{rhs}.}
...
points. First, the variational forms defining the predictor step for
...
those figures. The implementation of the weak forms for the predictor
equation for the composition, and the predictor and corrector
equations for the temperature then build on these basic functions. The
...
def energy_correction(mesh, dt, u, u_, T_):
    ...
    \caption{Definition of variational forms for one correction step
      ...
      functions \code{lhs} and \code{rhs}.}
...
\begin{figure}
  \begin{center}
   \includegraphics[width=0.95\columnwidth]{chapters/vynnytska/png/rms_velocity.png}
...
We would like to stress that compositional variation as presented in
the scenario in question requires solving
equation~\eqref{vynnytska:eq:transdif}.  Solving this equation is
challenging for the numerical schemes as it requires tracking of sharp
interfaces.  Field, markers and marker chain approaches are proposed
in the literature to trace compositional
discontinuities~\citep{IsmailZadehTackley2010}. We have here used the
field approach to represent compositional heterogeneity as it is
easily implemented by the means of discontinuous Galerkin
methods. Additionally, a filtering scheme is included in order to
minimize numerical dispersion error.
