...
forms for the G2 discretization of the UC model (see figure
...
figure Figure~\ref{code:FFC_ElasticSmoother}). We define the following
...
\begin{figure}
  \centering
  \includegraphics[width=11cm]{chapters/hoffman-2/pdf/fsi_plot.pdf}
  \caption{Displacement of tip of structure over time for FSI benchmark and use case.}
  \label{fig:fsi2D_plot1}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=10cm]{chapters/hoffman-2/png/fsi3_vel_050.png}\\
  \includegraphics[width=10cm]{chapters/hoffman-2/png/fsi3_theta_050.png}\\
  \includegraphics[width=10cm]{chapters/hoffman-2/png/fsi3_vel_250.png}\\
  \includegraphics[width=10cm]{chapters/hoffman-2/png/fsi3_theta_250.png}
...
and \citep{HoffmanJanssonStockli2011}. The displacement over time of
the tip of the structure is plotted in
figure \ref{fig:fsi2D_plot1}. The velocity field and phase function
...
\item[Parameters] \ \\ Simple parameters specifying material properties and i.e. numerical tolerances are given
in a parameter file.
\item[main program] \ \\ The user implements the main program and
  declares and passes data to to the solver.
\end{description}

The four parts are demonstrated in code examples in figures \ref{code:UseCasePart1} to \ref{code:UseCasePart4}.
...
public:
  bool inside(const real* p, bool on_boundary) const
  {
    return on_boundary && (p[0] < Geo::xmax - bmarg ||
                           p[1] < Geo::ymin + bmarg || p[1] > Geo::ymax - bmarg);
  }
};
...
public:
  ...
  {
    bool condition =
      (((flag_start_x - bmarg) < p[0] && p[0] < (flag_end_x + bmarg)) &&
       ((flag_start_y - bmarg) < p[1] && p[1] < (flag_end_y + bmarg)));
...
void solve(Mesh& mesh, Checkpoint& chkp, long& w_limit, timeval& s_time)
{
  ...
  NSESolver psolver(mesh, node_normal, f, beta, aero_f, bc_mom, bc_con, chkp, w_limit, td, ``primal'');
  psolver.solve();
}
