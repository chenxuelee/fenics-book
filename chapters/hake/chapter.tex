...
human body \cite{AlbertsBrayLewisEtAl2002}. In particular, \Ca controls
cell death, neural signaling, secretion of different chemical
substances, and the focus of this chapter: the contraction
of cells in the heart.\par
...
models. Numerical methods implemented in \pydolfin solving the partial
...
\index{cytosol}In a healthy heart, every heart beat originates in the
...
\index{diffusion constant} The dyadic cleft is the volume in the
...
\index{advection-diffusion} \index{Gouy-Chapman} \index{Nernst-Planck
  equation} We will use Fick's second law to model diffusion of \Ca in
...
The boundary, $\partial\Omega$, is divided into 4 disjoint boundaries,
...
% approximation \cite{Luo_1994_1071,Camp_1988_267}. The first flux is
% given by,
\begin{equation}
...
\index{stochastic channel} \index{Markov chain model} \index{Discrete
  state} \index{propensity functions} Discrete and stochastic Markov
...
inactivation \cite{JafriRiceWinslow1998,GreensteinEtAl2002}. The state
...
\label{sec:hake:ryr}
\index{ryanodine receptor} RyRs are \Ca specific channels that are
...
%  \vspace{-1.5em}
%  \caption{Python code for the assembly of the matrices and vectors from Eq.~(\ref{eq:hake:matrices})-(\ref{eq:hake:vector}).}
...
The following \pydolfin code assembles the matrices and vectors from
...
The \pydolfin code that assembles the SUPG part of the problem is
...
%  \vspace{-1.5em}
%  \caption{Python code for solving the system in Eq.~(\ref{eq:hake:algebraic-equation}), using the assembled matrices from the two former code examples from Fig.~\ref{fig:hake:assembly-algorithm}-\ref{fig:hake:SUPG-assembly-algorithm}.}
...
In Fig.~\ref{fig:hake:traces_mesh_2}, the traces from Mesh 2 are
...
% numerical solution for the advection-diffusion problem presented in
% Eq.~(\ref{eq:hake:full_system}). Using a finer mesh close to $z=0$
...
% proteins at these points.\par
%\label{sec:hake:solution}
%\begin{figure}
%  \centering
%  \LVerbatimInput[fontsize=\scriptsize,frame=lines,
%  framerule=0.4mm,
...
%  \label{fig:hake:time-stepping-algorithm}
%\end{figure}
\begin{figure}
...
In Fig.~\ref{fig:hake:time-line}, we show an example of a possible
...
\packagefont{Python} packages.\par
