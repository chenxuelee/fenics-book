...
by~\cite{Rivenaes1992, Rivenaes1993}:
...
\end{equation}
%The term \emph {dual} lithology is not to be confused with the dual problem below.
...
by~\cite{ImhofSharma2007}, which is considered impractical, we do not
...
Landweber's steepest decent algorithm~\cite{Landweber1951} iterates the following sequence until convergence:
\goodbreak
\bigskip
...
From the Ansatz \eqref{eq:schroll:ansatz} and the approximation \eqref{eq:schroll:approx}, it follows that
\begin{equation}
 -2a \bar\gamma +b = \nabla_p J(p^{k-1}) \cdot d^k .
\end{equation}
...
kind of duality arguments is~\cite{ChaventLemmonier1974}.
...
selecting a suitable function space, the equation in variational form reads
\begin{equation}
...
\end{equation}
such that $a(u_{\alpha_i}, \omega) =  J_{\alpha_i}$ and by using the dual solution $\omega$ in \eqref{eq:schroll:A}
\begin{equation}
...
in~\cite{Schroll2008}.  For completeness, not for computation(!), we
...
functional \eqref{eq:schroll:output} for possible calibration of the
dual lithology model \eqref{eq:schroll:model} to ``observed'',
actually generated synthetic, data.  We consider the PDE
system \eqref{eq:schroll:model} with discontinuous coefficients
\begin{equation}
 \alpha = \left\{
...
\begin{figure}
  \begin{center}
    ...
    \caption{Contours of $J$ and Landweber iterations. Optimal parameters (green), reference parameters (red).
      Clean data left column, noisy data right. $\alpha$-iterations upper row, $\beta$-iterations lower row.}
    \label{fig2}
  \end{center}
\end{figure}
...
\begin{figure}
  \begin{center}
    ...
    \vspace{-0.7cm}
    \caption{Landweber iterations. Optimal parameters (green), reference parameters (red). Clean (left) and noisy data (right).}
    \label{fig3}
  \end{center}
\end{figure}
