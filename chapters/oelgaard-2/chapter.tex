...
are computed and assigned to the variables like \emp{K\_01} in the code
...
compile time and stored in the tables \emp{Psi\_w}, \emp{Psi\_vu\_D01} and
\emp{Psi\_vu\_D10} in Figure~\ref{oelgaard-2:fig:standard_code}.  After
...
code for evaluating \emp{F0} in Figure~\ref{oelgaard-2:fig:standard_code}
...
\caption{Part of the generated code for the bilinear form associated
         ...
         inverse of the Jacobian matrix and \emp{det} is the determinant
         of the Jacobian. The code to compute these variables is not
         shown.  \emp{A} holds the values of the local element tensor
         and \emp{w} contains nodal values of the weighting function~$w$.}
...
We now address optimizations for improving the runtime performance
...
\begin{description}
  ...
  which is also known as `loop hoisting' \index{loop hoisting}, can be
...
This optimization is switched on by using the command-line
option \emp{-f eliminate\_zeros}. Code for the weighted
...
\caption{Part of the generated code for the weighted Laplacian using
         ...
         \emp{-f eliminate\_zeros}.  The arrays \emp{nzc0} and \emp{nzc1}
         ...
         tables with derivatives of basis functions \emp{Psi\_vu\_D01} and
         \emp{Psi\_vu\_D10} from Figure~\ref{oelgaard-2:fig:standard_code}
         with one table (\emp{Psi\_vu}).}
...
The \ffc{} command-line option \emp{-f simplify\_expressions}
...
command-line option \emp{-f precompute\_ip\_const} should
...
that about half of the temporary values in the array \emp{B} only depend
on the loop index \emp{j}, and they can therefore be hoisted, as we have
done for other terms in previous sections.  Another approach is to unroll
the loops with respect to \emp{j} and \emp{k} in the generated code.
...
we will use \emp{-zeros} as shorthand for the
...
The operation counts for the weighted Laplace
...
where no g++ optimizations are used (the \emp{-O0} flag), the runtime for
...
The \emp{-O3} option does not appear to improve the runtime noticeably
beyond the improvement observed for the \emp{-O2 -funroll-loops} option.
Using the \ffc{} optimization option \emp{-zeros} alone for this form
...
runtime, even when combining with the option \emp{-simplify}, which has
...
there is a significant difference in runtime for the \emp{-ip} and
\emp{-basis} options, even though they involve the same number of flops.
...
\emp{-ip} or \emp{-basis} combined with g++ option \emp{-O2}, in which
case the runtime has improved by a factor of 7.23 compared to standard
quadrature code with no g++ optimizations.

The operation counts and \ffc{} compile time for the bilinear
...
When the g++ optimization \emp{-O2} is switched on, this effect
...
beyond \emp{-O2} does not seem to provide any further improvements in
run time.  For the hyperelasticity example, the option \emp{-zeros}
...
that the \emp{-simplify} option performs particularly well for this
...
the \emp{-simplify} option very efficient since many terms are common
and can be precomputed and hoisted.  For the hyperelasticity form,
the optimal combination of optimizations is \ffc{} option \emp{-basis
-zeros} and g++ option \emp{-O2 -funroll-loops} which improves the
...
As demonstrated in the previous section, a given type of optimization may
...
is a good indicator of performance.  However, as we have shown in
