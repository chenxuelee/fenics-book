...
\noindent where $P_{q}\brac{T}$ denotes the space of Lagrange
...
\noindent where a change of variables from the reference coordinates
...
`loop hoisting'\index{loop hoisting}, can be found
...
\noindent where $\brac{\partial X_{\alpha_1} / \partial x_{\beta}}
...
To generate code with this optimization, the \ffc{} command-line
option \emp{-f precompute\_basis\_const} should be used.  Code
...
be reused inside the loop over the indices \emp{j} and \emp{k}.
...
where no g++ optimizations are used (the \emp{-O0} flag), the runtime
...
\emp{-zeros} alone for this form does not improve runtime performance.
...
difference in runtime for the \emp{-ip} and \emp{-basis} options, even
...
\emp{-O2}, in which case the runtime has improved by a factor of 7.23
...
combination of optimizations is \ffc{} option \emp{-basis -zeros} and
g++ option \emp{-O2 -funroll-loops} which improves the runtime
performance of the code by a factor of 3149 when compared to the case
where no optimization is used by either \ffc{} or~g++.

\editornote{HN: GNW, should -basis -zeros above be -simplify -zeros?
  That is what I gather from the figure.}
...
\noindent where $n_f$ is the number of premultiplying coefficients.
...
\noindent and the coefficient functions $f_{i} \in W_{h}$ with
...
\noindent where $q$ and $p$ denote the polynomial order of the
