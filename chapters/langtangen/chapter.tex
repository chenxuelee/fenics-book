...
\begin{equation} \label{langtangen:poisson1}
  \begin{split}
    - \Delta u &= f \qaud \mbox{in} \ \Omega,
\\
    u &= u_0 \qaud \mbox{on} \ \partial \Omega.
  \end{array}
\end{equation}
...
\index{finite element specifications}\index{CG finite element
  family}\index{Lagrange finite element family} Here, \emp{"CG"}
stands for Continuous Galerkin, implying the standard Lagrange family
of elements.  Instead of \emp{"CG"} we could have written
\emp{"Lagrange"}.  With degree 1, we simply get the standard linear
...
\emp{FunctionSpace}. Changing the second parameter to \emp{"DG"}
...
\\
    u(x,y) &= u_0(x,y) \quad \mbox{on} \  \partial\Omega\langtangenep
  \end{split}
\end{equation}
