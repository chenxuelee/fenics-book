...
program:\index{TrialFunction}\index{TestFunction}
\begin{python}
u = TrialFunction(V)
v = TestFunction(V)
\end{python}
...
by\index{DirichletBC}\index{Dirichlet boundary conditions}
\begin{python}
bc = DirichletBC(V, u0, u0_boundary)
\end{python}
...
we already have.  This is easily accomplished by\index{assemble}
\begin{python}
...
and
\emp{UnitCube}.
\index{UnitInterval}\index{UnitSphere}\index{UnitSquare}\index{Interval}\index{Rectangle}\index{Box}\index{UnitCircle}\index{UnitCube}
