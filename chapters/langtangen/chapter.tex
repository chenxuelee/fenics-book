...
\begin{eqnarray}
    ...
    - \frac{\partial u}{\partial n} &=& g \mbox{ on } \Gamma_N  %\label{langtangen:poisson:2D:DNbcN}\langtangenep
\end{eqnarray}
...
A, b = assemble_system(a, L, bc)
\end{python}
The \emp{assemble\_system} function incorporates the boundary conditions in
...
is placed on the main diagonal, and the right-hand side \emp{b} is
modified by subtracting the column in \emp{A} times the value of the
degree of freedom, and then the corresponding entry in \emp{b} is replaced by the
known value of the degree of freedom.) With \emp{condition.apply(A, b)}, the
matrix \emp{A} is modified in an unsymmetric way. (The row is zero'ed out
and 1 is placed on the main diagonal, and the degree of freedom value
is inserted in \emp{b}.)
...
components (see \emp{fenicsproject.org}). Some components are DOLFIN and
...
machines are available at the \emp{fenicsproject.org} website.
...
\emp{fenicsproject.org} and copying just five Unix
...
\citet{QuarteroniValli2008}, or \citet{Ciarlet2002}.
...
\citet{Langtangen2008,Langtangen2009a,Kiusalaas2010} are exceptions.
