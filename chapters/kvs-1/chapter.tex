\fenicschapter{A comparison of some common finite element schemes for the incompressible Navier--Stokes equations}
...
\emp{http://launchpad.net/nsbench/} and can be used to reproduce all
...
Another script \emp{bench} allows a user to iterate over all solvers
...
Goda~\cite{Goda197976}. The IPCS scheme is summarized in Scheme~2 and
the implementation is shown in Figure~\ref{fig:IPCS}. The IPCS scheme
as implemented here also differs from the Chorin scheme in that the
...
Chorin scheme. Note the importance of the term $\inner{v}{\nu
(\nabla \bar{u}_h^{\bigstar})^{\top} n}_{\partial\Omega}$ which arises
...
gives $\inner{\nabla q}{\dot{u}} = -\inner{q}{\nabla \cdot u} +
\inner{qn}{\dot{u}_{\partial \Omega}} = -\inner{q}{\nabla \cdot
  u}$. For non-constant Dirichlet boundary conditions, this assumption
is not valid. This issue is not addressed in~\cite{gueshe}, but it is
...
L3 = q*ps*dx + q*psi*dx - nu*q*div(u1)*dx
    \end{python}
    \caption{Implementation variational forms for the CSS
...
the pressure Schur complement (GRPC), see~\cite{T-99}.  The
...
pressure Schur complement $B^T (M + \deltat N)^{-1} B$. It is well known
that if an explicit scheme is used for convection, then order-optimal
solution algorithms for both $M + \deltat N$ and $B^T (M +
\deltat N)^{-1} B$ are readily available,
see references~\cite{T-99,M-W-04,M-W-09,CahouetChabard88}. In fact
$L^{-1} \approx \deltat M_Q^{-1} + A_Q^{-1}$, where $M_Q$ and $A_Q$
are the mass and stiffness matrices associated with the pressure
...
Chapter~[mardal-4]. In the implementation, we have chosen to exclude
the convective term in the preconditioners $K$ and $L$. The GRPC
scheme is summarized in Scheme~6 and the implementation is shown in
Figure~\ref{fig:grpc_code}.
...
clear difference between \css{1} and \css{2}.
...
\begin{figure}
  \begin{center}
    \fbox{
      \includegraphics[width=50mm]{chapters/kvs-1/eps/periodic_v.eps}
      \includegraphics[width=50mm]{chapters/kvs-1/eps/periodic_p.eps}
    }
...
\begin{figure}
  \begin{center}
    \fbox{\includegraphics[width=14cm]{chapters/kvs-1/eps/cylinderandgeoattime5nr2.eps}}
...
We next consider a problem described in~\cite{steinmann} where an
...
\begin{figure}
  \begin{center}
    \includegraphics[width=10cm]{chapters/kvs-1/eps/new_beltrami_illustration.eps}
...
\begin{figure}[htbp]
  \begin{center}
\fbox{\includegraphics[width=14cm]
{chapters/kvs-1/eps/aneurysm_at_end_css1.eps}}
