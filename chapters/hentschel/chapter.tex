...
This chapter builds on the software tools described in Chapter~[kvs-1], and
...
the skull basis, foramen magnum \index{foramen magnum}, connects the cranial
...
Several studies~\cite{Quigley2004,HaughtonKorosecMedowEtAl2003} report that the measured flow at foramen magnum
is abnormal in the sense that the flow contains high speed jets and also bidirectional flow. 
Computational fluid dynamics (CFD) simulations have demonstrated have related the abnormal flow to 
abnormal pressure~\cite{RoldanHaughtonWiebenEtAl2008,Simula.SC.648,Simula.SC.522}. 
Many theories have been proposed to describe the relation between the Chiari I malformation and
syringomyelia. It is however a problem to explain the relatively large distance between the 
Chiari I malformation and the cyst.  
It is the purpose of this
chapter to describe the implementation of such a CFD solver in
\fenics\ and investigate the flow characteristics. 
Note that the Navier-Stokes solvers are discussed in
...
\nabla \cdot \mathbf{v} &=& 0, \quad \in \Omega,
\end{eqnarray*}
...
The mesh boundaries at the inlet cross section, the outlet cross section, and the SAS boundaries are defined by the respective classes \emp{Top}, \emp{Bottom}, and \emp{Contour}. They are implemented as subclasses of \emp{SubDomain}, similarily to the given example of \emp{Top}.
\begin{python}
class Top(SubDomain):
...
\emp{Function}, enabling parameter dependencies to be evaluated at run-time. To invoke an object of \emp{Pulse}, the global function
...
Exterior forces on the Navier-Stokes flow are defined in the object variable \emp{f}. Since gravity is neglected in the current problem formulation, the force function \emp{f} is defined by a constant function \emp{Constant} with value zero on the complete mesh.
...
\subsection{Generation of Meshes with Gmsh} 
\index{Gmsh}
...
We define a characteristic length scale \emp{lc}, which is used in the definition of each point \emp{Point}, which 
...
We may then change to "Mesh modus" in the interactive panel and press \emp{3d} to construct the mesh. Pressing \emp{Save} will save the mesh with the under then name "filename", with extension \emp{msh}. For use in \dolfin, we apply the \dolfin\ converter.
...
The presented results were simulated on meshes of length 5 cm with 30 layers in z-direction and three layers on the side boundaries. The complete code can be found in \emp{csf/mesh\_generation/gmsh}.
...
In the first example we represent the spinal cord and the surrounding dura mater as two straight cylinders, created using Gmsh. 
%These cylinders can easily be generated by using NetGen ~\cite{SchoberlGerstmayrGaisbauer} or Gmsh ~\cite{GeuzaineRemacle}.  In NetGen meshes can be constructed by adding or subtracting geometrical primitives from each other. It also supports \dolfin\ mesh generation. \paragraph{Results.}
...
the three different solvers. Strangely, it appears that the G2 solver requires a 30\% bigger
pressure gradient between the top and the bottom in this case. However, the 
G2 solvers has also produced some peculiar results in Chapter~[kvs-1]. We also remark that
G2 was about 15 times slower than Chorin and IPCS. However, of main interest here is how
...
According to ~\cite{Lopes2007,AlperinMazdaLichtorEtAl2006}, the present flow is inertia dominated, meaning that the shape of the cross section should not influence the pressure gradient. Changing the length of vectors describing the ellipse from
\begin{bash}
c = 0.4;
d = 0.4;
\end{bash}
...
that pressure instabilities is quickely damped out under realistic Reynolds and 
...
down in the spinal canal. We also observed that the pressure computed using the G2 method
