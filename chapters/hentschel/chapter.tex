...
The CSF is a clear water-like fluid that occupies the subarachnoid
...
cavities, often called syrinxes or systs, within the spinal
cord, \index{spinal cord} a condition called syringomyelia. The exact
...
the size of the cavity in the spinal canal and patients usually report
...
different solvers Chorin, IPCS, and G2, c.f. Chapter~\ref{chap:kvs-1}.
...
Navier--Stokes \index{Navier--Stokes} equations for an incompressible
Newtonian fluid,
\begin{eqnarray}
...
solvers and schemes implemented. In this chapter we concentrate on the
use of these solvers in a few examples. Notice, however, that we use
first order velocity elements. The code can be found
in \emp{csf\_flow.py}.
...
\end{python}
To define the domain correctly, we override the base class' object
...
during each cardiac cycle. For the average human, we assumed a heart
rate of 70 beats per minute. Furthermore, we defined the cross
...
mesh in viper \emp{unix>viper meshname.xml}. Typing \emp{o} prints the
...
formulation, the force function \emp{f} is defined by a constant
function \emp{Constant} with value zero on the complete mesh.

After initializing the subdomains, \emp{Top}, \emp{Bottom}
and \emp{Contour}, they are marked with reference numbers attributed
to the collection of all subdomains \emp{sub\_domains}.
...
should converge. For too small time steps, however, it can lead to an
...
Being a subclass of \emp{ProblemBase}, \emp{Problem} overrides the
object functions \emp{update} and \emp{functional}. The first ensures
