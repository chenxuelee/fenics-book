...
The presented results were simulated on meshes of length 5 cm with 30 layers in z-direction and three layers on the side boundaries.The complete code can be found in \emp{mesh\_generation/FILENAME}%FIXME
.\\
...
def get_sine_input_function(V, z_index, HR, HR_inv, v_max):
	...
	return Sine(V)
\end{code}
that is called instead of \emp{get\_pulse\_input\_function} in the function named \emp{boundary\_conditions}:
\begin{code}
