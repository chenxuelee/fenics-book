...
Instant is a small Python module for just-in-time compilation (JIT)\index{JIT} (or inlining) of
C/C++ code.
Instant accepts plain C/C++ and is therefore conventiently combined with the code generating tools in Dolfin, FFC, and SFC.   
...
Similarly, \emp{Expression}s and \emp{SubDomain}s may be compiled using JIT compiler in Dolfin. 
In all the mentioned cases, Instant is employed for to obtained a seemless integration of C++ and Python,  
see also the Chapters~[alnaes-3,logg-1,mardal-2]. 

Instant relies on SWIG~\cite{www:swig} for the generation of wrapper
code needed for making the C/C++ code usable from
Python~\cite{www:python}.  The code to be inlined, in
...
We refer to its website for
a comprehensive user manual. Some issues concerning SWIG is also 
described in Chapter~[mardal-2]. 
...
deals with NumPy arrays can be found in Section \ref{wilbers:sec:arrays}.
...
The complete code can be found in \emp{\$examples/ex3.py}. 
...
There are basically three different types of errors you can encounter when using Instant. 
These are 1) errors caused by non-compilable c++ code, 2) errors caused by wrong usage of SWIG, 
...
Traceback (most recent call last):
      ...
      see '/tmp/tmpZ4M_ZO2010-11-9-08-24_instant/instant_module_dff946511241aab327593a2d71105c5fc/compile.log'
\end{python}
The error message from the compiler is located in the file \emp{compile.log}, but this compile error
will refer to line numbers in the wrapper code generated by SWIG. You should still be able to locate
the C++ error. 

The second type of error occurs when SWIG is not able to parse the code. These errors  
...
gcc: no input files
\end{python}
In this case SWIG will not be able to generate a wrapper file and hence the extension module will not be built either. 
...
to disk failure, some shared libraries might be missing from \emp{\$LD\_LIBRARY\_PATH} and so on. 
...
\end{python}
In this case it is advantegous to make a local cache, using the  \emp{cache\_dir="test\_cache"}, and go 
...
\emp{ex4\_\-cache.i} in this example. Then the Distutils file \emp{setup.py} is generated 
...
directory. The file \emp{\_\-\_\-init\_\-\_\-.py} imports the module \emp{ex4\_\-cache}.
...
The script \index{\emp{instant-showcache}}\emp{instant-showcache} shows the modules located in the
Instant cache.
...
array (an \emp{int}), and the array pointer. The array pointer 
can have several types, but the default is \emp{double}). For 2-dimensional arrays we need three strings, two
...
\paragraph{Input/Output Arrays}
Instant supports for both input, output and in-place 
arrays. Default behaviour is to treat the arrays as in-place arrays, as 
long as the input are Numpy arrays. Python lists and sequences are converted to Numpy arrays automatically.  
The following code shows an example where we calculate matrix-vector
...
It is possible to work with arrays that are more than 3-dimensional. 
...
resulting code is copied to the Instant cache unless \emp{cache\_\-dir} is set to
...
\item \emp{init\_\-code}
  \begin{itemize}
    ...
    \item Comment: Code that should be executed when the Instant module is
      imported. This code is used in the initialization of the module. An
...
\item \emp{additional\_\-definitions}
  \begin{itemize}
    ...
    \item Comment: Additional definitions (typically needed for inheritance)
      for the interface file. These definitions should be given as triple-quoted
...
\item \emp{additional\_\-declarations}
  \begin{itemize}
    ...
    \item Comment: Additional declarations (typically needed for inheritance)
      for the interface file. These declarations should be given as triple-quoted
...
\item \emp{arrays}
  \begin{itemize}
    ...
    \item Comment: A nested list describing the C arrays to be made from the NumPy arrays.
        ...
        If the NumPy array har more than three dimensions, the list should
...
possible to inline one C/C++ function each time. One argument is mandatory; the C/C++ code to be compiled.
...
\index{Cython}Cython~\cite{Cython}, and \index{F2PY}F2PY~\cite{Peterson}.
