...
keyword argument \emp{modulename} to see more clearly ie., 
\begin{python}
from instant import inline
code = r'''
double add(double a, double b)
...
and the directory \emp{build} is also created by Distutils.  
...
directory. The file \emp{\_\-\_\-init\_\-\_\-.py} imports the module \emp{ex4}.
...
Instant supports for both input, output and in-place arrays. 
...
multiplication $x = Ab$. The integer matrix $A$ and double vector $b$ is marked as input, while 
the double vector $x$ is output. The code is found in 
file \emp{\$examples/ex5.py}. 
\begin{python}
...
We mention \index{Weave}Weave~\cite{www:weave},
...
other mentioned packages, we refer to~\cite{WilbersLangtangenEtAl2009,www:weave}. 
