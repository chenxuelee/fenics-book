...
Similarly, Instant is used in Dolfin to JIT-compile \emp{Expression}s and \emp{SubDomain}s. 
See the Chapters~[alnaes-3,logg-1,mardal-2] for more information on these topics. 

Instant relies on SWIG~\cite{SWIG} for the generation of wrapper
...
\end{python}
When runned this script produces the following output: 
\begin{python}
 > python ex1.py 
--- Instant: compiling ---
...
The first time the Python script is runned, it will use a few second to compile the C code.  
...
In Table \ref{wilbers:fig:speed-up} we compare the above mentioned code with
...
using Instant instead of NumPy~\cite{WilbersLangtangenEtAl2009}.
...
the following code preprocessor directives like '\emp{\#pragma omp \ldots}' are OpenMP directives and
OpenMP functions always start with \emp{omp}.
...
first element of \emp{system\_\-headers} could have been omitted if we 
used \emp{inline\_\-module\_\-with\_\-numpy} instead of \emp{build\_\-module}. 
...
You should still be able to locate the C++ error, by looking at the error message in \emp{compile.log}
and the file containg the wrapper code (named \emp{*\_wrap.cxx}) in the temporary directory.
...
gcc: no input files
\end{python}
SWIG reports that it is unable to parse the Instant generated interface file (named \emp{*.i}) 
