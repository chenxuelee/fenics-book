...
\editornote{This section needs to be updated if/when we change the \emp{NewtonSolver}
to make the solver and preconditioner variables parameters.}
...
\editornote{This section needs to be updated if/when the interface is
  changed to require default values for variables to be supplied to
  the constructor.}
...
appropriate since in the UFL form language, \emp{ds = ds(0)}. The default
domain of integration is the domain marked by~\emp{0}. One could also
have used \emp{ds(1)} and marked the Neumann boundary by~\emp{1}.
