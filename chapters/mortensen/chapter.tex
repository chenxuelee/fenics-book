...
Chapter~\ref{chap:kvs-1} for laminar flows. In this chapter focus is put more
...
with $\vec{f}$ (the gravitational force or the Coriolis forces
...
obstacles, sudden pressure fluctuations, or even a sound waves,
...
functions and denote by $\Hdivnull$ the subspace of
...
($\vec{u}\in \Hdivnull$), the following result can be obtained for
...
holds for any velocity field $\vec{u} \in L^2$, and not just the
divergence free $\vec{u} \in \Hdivnull$. This is an important
...
pressure. Let us denote this tentative or intermediate solution by
...
the same way as described in the benchmark Chapter~\ref{chap:kvs-1}, but the
...
%     L0 = rhs(F)
% ecod
% Here u0 and u1 represent known velocity fields at time steps $n-1$ and $n$ respectively and p0 is the latest available pressure. The pressure correction (\eqref{eq:mortensen:PC}) is implemented as
% bcod
...
\begin{itemize}
 \item[(1)] Split \emp{F} into accumulation $(\partial \vec{u}/\partial t)$,
...
%  Note that the convection matrix \emp{Ac}, which is reassembled every time
...
\caption{ CPU times (total/inside time loop) for the fully coupled and
  ...
  parenthesis show timings for the solver and assembler respectively.
}
\vskip 1.5ex
\centering
...
time steps and a total integration time of 0.5. The naive
...
the \emp{eval} function with the equivalence of
