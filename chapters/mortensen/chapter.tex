...
the NS equations have already been discussed in
Chapter~\ref{chap:kvs-1} for laminar flows. In this chapter, focus is
put more directly on flow energy and energy conservation, features of
...
memory efficient, but incurs a splitting error due to the uncoupling
...
The solvers and problems are implemented to fit into the problem
solving environment described in the benchmark
...
methods~\citep{CanutoHussainiQuarteroniEtAl1988} in homogeneous flows
or higher-order central finite differences or spectral element
methods~\citep{Blackburn2009} for more geometrical flexibility in
inhomogeneous flows. The results of carefully executed DNS have in the
fluid mechanics community the same status as carefully executed
...
smallest scales of turbulence; that is, the scales that are
responsible for dissipating energy. Yet another complication in
non-periodic flows is to describe inflow and outflow boundary
...
viscous parallel shear flow. Although the Orr--Sommerfeld equation
only represents one simplified class of laminar-to-turbulence
transition, it nevertheless constitutes a powerful method to assess
numerical schemes since it provides an analytical transient solution
to the NS equations that remains non-trivial for long integration
times. The Orr--Sommerfeld test case will be further discussed in
...
fractional step (projection) methods for the NS equations the
divergence constraint is not always fulfilled, at least not for
intermediate velocity fields. With the skew-form it is ensured that
this divergence flaw does not propagate and contaminate the (of
...
\eqref{eq:mortensen:NS_FS}, \eqref{eq:mortensen:PC},
...
equations are very much simpler than the original coupled problem
...
different. Here we introduce \emp{u} and \emp{p} for the unknown
...
\emp{up} and defined on the composite space of the velocity and
...
integration loop; that is, the total time minus the time it takes to
set up the problem for looping. The two numbers in parenthesis show
...
the source code can be found in the file \emp{OrrSommerfeld\_eig.py}
that comes with the chapter. For the initialization of DOLFIN
\emp{Functions} with the Orr--Sommerfeld solution, a subclass called
\emp{U0} of the DOLFIN class \emp{Expression} is implemented such that
it solves the eigenvalue problem on creation and overloads the
\emp{eval} function with the equivalence of
\eqref{eq:mortensen:channel}. To initialize the specified initial
velocity field we need to create an instance of the \emp{U0} class and
interpolate it onto the appropriate function space: \emp{V} for
fractional step and \emp{VQ} for the coupled solver. The procedure for
