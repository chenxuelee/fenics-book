...
\editornote{Use Re and Im for real and imaginary parts, respectively.}

\editornote{Figures are referred to be (a), (b), etc., but (a), etc. does not
appear below figures.}
...
convective term,
\editornote{'. . the convection of any vector . .. does not make sense.}
\begin{equation}
B(\vec{u},\vec{a}) =  \nabla \vec{a} \, \vec{u},
\end{equation}
...
\editornote{$\vec{u} \in L^2$ is not correct.}
...
and contaminate the kinetic energy of the flow.
...
\end{equation}
Evidently, since $\nu \ge 0$, energy should decay within the domain. For
...
\editornote{The above is unclear.}
...
time level $n-\alpha$. For $\alpha =1/2$ we have a centered scheme
...
\editornote{`to place the pressure using a staggered grid in time' does not make sense.}
...
discretizations of the convection term $B$.  The explicit Adams-Bashforth
scheme \eqref{eq:mortensen:EX} below is chosen primarily because
...
\end{align}
\editornote{The above could be better structured to avoid
using eqn numbers before the eqn has been presented.}
The middle scheme \eqref{eq:mortensen:IM1} is first-order, whereas the
...
-\nabla\Phi,
\end{equation}
coupled with the requirement that the new velocity must fulfil:
\begin{equation}
...
and solve each scalar equation separately.
...
divergence-free energy conservation. Hence, since the velocity fields
...
Chapter~\ref{chap:kvs-1}, and are thus not repeated here.

\subsection{Speed-up}
\label{sec:mortensen:speed-up}

\editornote{Section title is incomplete. Speed up w.r.t what?}
...
\editornote{What `mentioned properties'?}
...
Section~\ref{sec:mortensen:TG}) with $Re=100$ on a $16^3$ mesh using
four time steps and a total integration time of~$0.5$.
\editornote{What is a `$16^{3}$ mesh'?}
The naive implementations referenced in Table~\ref{tab:mortensen:speed-up}
use one line of code style and lhs/rhs to extract forms for the numerical
schemes.
\editornote{What is a `one line of code style'?}
This corresponds roughly to the code presented at the beginning of
...
and $\kappa$ is the amplitude of the perturbation.
...
the domain, using the Dirichlet boundary conditions.
\editornote{'. . . using he Dirichlet boundary conditions.' doesn't make sense.}
The resulting variational form is
\begin{equation}
...
the forward Euler method (left). Most remarkable, though, is the exact
conservation of kinetic energy achieved by the skew form.
\editornote{Why is this `Most remarkable'. Isn't it expected?}
...
$\text{exp}(i(\alpha x-\lambda t))$ (real part understood),
\editornote{What does `real part understood' mean?}
where $\lambda$ is an eigenvalue (the complex frequency), $\alpha$
...
\ref{fig:mortensen:OS_init} (b). Note the eigenvalue in white ($\lambda =
...
\editornote{Use better description than `in white'. If it's white, it will
be invisible.}
...
\emp{U0} class and interpolated in the appropriate function space:
...
p_ = Constant(0.)
\end{python}
any parameter required by the expressions in the \emp{U0} class.
\editornote{`any parameter required by the expressions . . ' ?}
In the
end, the pressure is set to zero, which finalizes the initialization
process. For the coupled solver, \emp{VQ} is used in place of \emp{V}
...
To validate the more interesting long-term performance of the solvers,
...
and time step with a cell-vertex based finite volume solver.
\editornote{'. . with circles is the result . . '?}
\editornote{What does `cell-vertex based finite volume solver' mean?}
...
use $30 \times 30$ homogeneous elements of order size size in both $x$
and $y$-directions and 144 planes in the $z$-direction that is solved
using Fourier expansions. The time step in the second-order time stepping
routine is set to 0.005.
\editornote{Make clear that simulation details are for Semtex.}
...
implicit discretizations in correlation with the fractional step method
