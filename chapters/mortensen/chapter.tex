...
$\tilde{\vec{u}}$ and $\Bar{\vec{u}}$ are named \emp{u\_tilde}
...
Section~\ref{sec:mortensen:TG}) with $Re=100$ on a $16^3$ mesh using four
...
beginning of Section \ref{sec:mortensen:impl_fenics} for the coupled
...
The variational form of Burgers' equation transferred to FEniCS is
...
comes with the chapter. For the initialization of DOLFIN \emp{Functions}
...
\emp{U0} class and interpolate it onto the appropriate function space:
...
p_ = Constant(0.)
\end{python}
any parameter required by the expressions in the \emp{U0} class. In the
...
instability than the lower-order finite volume method, which is
...
total energy. Furthermore, the additional accuracy earned through using
