...
q}, \emp{P\_element = DiscontinuousLagrange} and {\tt P\_order = q-2}.
...
For our tests, we use an $n \times n$ unit square mesh with a criss-cross
pattern, as illustrated in Figure~\ref{fig:terrel:meshsolution}(a). As
a first test case, we use the following analytical solution of the
Stokes equations:
%
\begin{align}
...
they are computed on a finer mesh that has a comparable number of degrees
of freedom to the fourth-order Taylor--Hood element.
