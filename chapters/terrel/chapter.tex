...
pressure term in~\eqref{eqn:terrel:StrongStokes} is the opposite of what
is commonly used. We choose this form as it will lead to a symmetric
matrix operator.
...
\editornote{AL: Remove comment on residual based stabilization here since it was unclear.}
...
\editornote{AL: Why criss-cross? Please comment.}

For our tests, we use an $n \times n$ unit square mesh with a
criss-cross pattern, as illustrated in
Figure~\ref{fig:terrel:meshsolution}(a). As a first test case, we use
the following analytical solution of the Stokes equations:
%
\begin{align}
...
\editornote{AL: Unclear which test case the results are for: analytic or lid-driven cavity? Please add info to all tables and figures.}
...
\editornote{AL: Missing conclusion that TH produced the smallest velocity errors in all cases?}
