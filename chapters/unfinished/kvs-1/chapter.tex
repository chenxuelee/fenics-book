\fenicschapter{A comparison of some finite element schemes \\ for the incompressible Navier--Stokes equations}
              {A comparison of finite element schemes Navier--Stokes}
...
implemented are Chorin's projection scheme by~\cite{Chorin1968}
and~\cite{Temam1969}, the incremental pressure correction scheme
(IPCS) by~\cite{Goda1979}, the consistent splitting scheme (CSS)
by~\cite{GuermondMinevShen2006}, a least-squares stabilized Galerkin
scheme (G2) by~\cite{JohnsonHoffman2007}, and a saddle-point solver
based on a Richardson iteration on the pressure Schur complement
(GRPC) as described in~\cite{Turek1996}.
...
throughout this chapter. The problems, derived from
the \emp{ProblemBase} class, define the mesh, initial and boundary
...
over all problems and all solvers. As an example, the following
command may be used to solve the channel test problem with all solvers
on a mesh refined twice:
\begin{bash}
...
correction scheme was first proposed by~\cite{Chorin1968}
and~\cite{Temam1969}. For simplicity, we will here refer to this
...
by~\cite{Goda1979}. The IPCS scheme is summarized in Scheme~2 and the
...
in~\cite{GuermondMinevShen2006,GuermondShen2003}, is derived
...
the pressure~$p$ by testing the momentum equation~(\ref{eq:ns,mom})
...
follows. Multiply the momentum equation~(\ref{eq:ns,mom}) by $\nabla
q$ for $q
...
is not valid. This issue is not addressed in~\cite{GuermondShen2003}, but it is
...
\begin{equation}
  SD_{\delta}
  = \renni{\nabla v \cdot u_h^{n-\frac{1}{2}}}{\delta_1 \nabla u_h^{n-\frac{1}{2}} \cdot u_h^{n-\frac{1}{2}}}
...
\begin{eqnarray*}
    ...
    + \renni{v}{\nabla u^{n-\frac{1}{2}}_h \cdot u^{n-\frac{1}{2}}_h}
...
discrete divergence, and $b$ is a vector depending on the solution on
...
the pressure Schur complement (GRPC); see~\cite{Turek1999}. The
...
+ \deltat N)^{-1} B$ are readily available; see
references~\cite{Turek1999,MardalWinther2004,MardalWinther,CahouetChabard1988}. In
...
functional are available in~\cite{PanditKalitaDalal2007}, where a
reference value of $\min\psi = -0.0585236$ is reported, and
in~\cite{ChudanovPopkovChurbanovEtAl2007}, where a value of $\min\psi
...
computed the solution using the spectral element code~\cite{Semtex}
...
third order implicit discretization and a very short timestep was used
...
\end{equation}
For a more thorough description, see~\cite{White1999}
or~\cite{White1991}. Figure~\ref{fig:streamfunction} shows how to
...
described in~\cite{CanutoHussainiQuarteroniEtAl2007}, which is a
...
\begin{figure}
  \begin{center}
    ...
    \caption{Analytical solution for the Taylor--Green vortex test
...
\begin{equation} \label{eq:cyl_inflow}
  u(0, y, t) = (4 U_m y (H - y) \sin(\pi t/8)/H^{4}, 0), \quad t \leq 8,
\end{equation}
...
\begin{figure*}
  ...
  \caption{Magnitude of the velocity for the cylinder test problem at $t = 5$.}
...
We next consider a problem described in~\cite{EthierSteinmann1994}
...
and $d=\pi/2$ as in~\cite{EthierSteinmann1994}. The kinematic
...
saccular aneu\-rysm (see Chapter~\cite{chap:kvs-2}). The diameter of
...
\begin{figure}[htbp]
  ...
  \caption{Solution for the aneurysm test problem at final time $T = 0.05$ ms.}
  \label{fig:aneurysm}
\end{figure}
...
As pointed out in~\cite{GuermondMinevShen2006}, the fractional step
