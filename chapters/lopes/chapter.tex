...
\emp{http://ptmat.fc.ul.pt/$\sim$ndl/}).
...
e.g., ~\citet{WalkleyBerzins2002}, ~\citet{WooLiu2004a} as well
as~\citet{WooLiu2004b} for finite element discretizations of BEV
models).  However, we only consider the wave surface Elevation and
velocity Potential (BEP) formulation (see, e.g.,
...
model mentioned in {\it i)}.  In the subsequent sections, only the
...
The shallow water wave models implemented and mentioned in {\it i)}
are the following:
\begin{itemize}
...
\emp{https://launchpad.net/dolfwave}. Some tools for the generation of
...
\emp{http://www.cmap.polytechnique.fr/$\sim$jouve/xd3d/}).  DOLFWAVE
...
\begin{enumerate}
\item[{\it i})] the kinematic boundary condition for the free water
  surface:
  \begin{equation}
...
\item[{\it ii})] linear dispersion characteristics, analogous to the
...
From~\eqref{eq:lopes:dissdisp}, the phase velocity, $\displaystyle
C=\frac{w}{k}$, for this dissipative and extended ZTC model is given
by:
...
where $K_1(t)$ is a generic time dependent function of integration.
...
of wave generation using a this source function is available at
\emp{dolfwave/demo/2HD/srcFunction}.
...
~\eqref{eq:lopes:fullrefl} and an extra artificial term, often called
sponge \index{sponge-layers} or damping layer, given by
$\nu\nabla^2\Phi$ (see equation ~\eqref{eq:lopes:ztc-b}), acting in a
neighborhood of the boundary $\Gamma$.  In this way, the reflected
energy can be controlled. Moreover, we can prevent unwanted wave
reflections and avoid complex wave interactions.  It is also possible
to simulate effects like energy dissipation by wave breaking.
...
\begin{itemize}
\item[(I)] when $\hat\omega$ is a pure imaginary number the solutions
...
\item[(III)] when a real solution is found for $\hat\omega$ but
...
equations~\eqref{eq:lopes:pc} is initialized by a fourth-order
