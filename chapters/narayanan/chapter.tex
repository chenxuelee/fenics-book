...
design of \twist, which is a PyDOLFIN (\cite{LoggWells2009}) module
...
Figure~\ref{narayanan:fig:continuumpotato}.

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.8\textwidth]{chapters/narayanan/images%
      /continuumpotato}
    \label{narayanan:fig:continuumpotato}
  \end{center}
...
\begin{equation}
  ...
  \label{narayanan:eq:balance-of-momentum}
\end{equation}
...
unit volume. Along with Equation~\ref{narayanan:eq:balance-of%
...
Equation~\ref{narayanan:eq:balance-of-momentum} is valid for all
...
\end{tabular}
\label{narayanan:table:straindefs}
\end{table}
...
\label{narayanan:code:kinematics.py}
\end{figure}

In \twist, each of the forms listed in
Table~\ref{narayanan:table:straindefs} have been implemented in the
file {\tt kinematics.py} in {\tt UFL} notation that closely resemble
their definitions above. Figure~\ref{narayanan:code:kinematics.py} presents a 
...
\end{split}
\label{narayanan:eq:secondpk}
\end{equation}

Using definitions such as the ones explicitly provided in
Equation~\ref{narayanan:eq:secondpk}, \twist{} computes the second
...
Table~\ref{narayanan:table:straindefs}. The base class for all
material models, {\tt MaterialModel}, encapsulates this
functionality. The relevant method of this class is provided in
Figure~\ref{narayanan:code:material_model_base.py}. The implementation
...
def SecondPiolaKirchhoffStress(self, u):
  ...
  return S
\end{python}
\label{narayanan:code:material_model_base.py}
...
code are shown in Figures~\ref{narayanan:code:svk} and
\ref{narayanan:code:mr}.
...
Equation~\ref{narayanan:eq:balance-of-momentum} with a test function 
...
form~\ref{narayanan:eq:weakform}, by looking for solutions in a finite
...
$\hat{\mathrm{V}^{h}} \subset \hat{\mathrm{V}}$.\footnote{We now
  ...
  Equation~\ref{narayanan:eq:weakform}, along with the various fields
...
\begin{equation}
  ...
  \bv \ds = 0.
\label{narayanan:eq:staticweakform}
\end{equation}
...
displacement field, $\bu$, for arbitrary material models, Equation~%
\ref{narayanan:eq:staticweakform} is just a nonlinear functional in
...
Figure~\ref{narayanan:code:staticmomentumsolver}.
...
\caption{The relevant section of the class {\tt
  ...
  linear momentum.}
\label{narayanan:code:staticmomentumsolver}
\end{figure}
...
(Equation~\ref{narayanan:eq:weakform}). The first of these is the
so-called cG(1) method (\cite{ErikssonEstepEtAl1996}). In order to
derive this method, Equation~\ref{narayanan:eq:weakform}, which is a
second order differential equation in time, is rewritten as a system
...
\end{split}
\label{narayanan:eq:cg1weakform}
\end{equation}
...
\end{split}
\label{narayanan:eq:cg1weakformapprox}
\end{equation}
...
solver class is presented in Figure~\ref{narayanan:code:cg1}. The code
closely mirrors the scheme defined in
\ref{narayanan:eq:cg1weakformapprox}, and results in a mixed system
...
\label{narayanan:code:cg1}
\end{figure}

The CG(1) scheme defined in Equation~\ref{narayanan:eq:cg1weak%
...
second order accurate and energy conserving.\footnote{This is
  demonstrated in Figure~\ref{narayanan:fig:energies} as part of the
...
\end{split}
\label{narayanan:eq:hhtdefs}
\end{equation}

\noindent Then, we insert the definitions in
Equation~\ref{narayanan:eq:hhtdefs} into the following form of the
...
\begin{equation}
  ...
  \label{narayanan:eq:newaccn}
\end{equation}

\noindent The acceleration solution to
Equation~\ref{narayanan:eq:newaccn} is then used in the
definitions~\ref{narayanan:eq:hhtdefs} to update to new displacement
...
problem. Figure~\ref{narayanan:code:dynamicrelease} is an example
showing this.
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics{chapters/narayanan/images/twistedcube.png}
  \end{center}
  \caption{A hyperelastic cube twisted by 60 degrees.}
    \label{narayanan:fig:twistedcube}
\end{figure}
...
Figure~\ref{narayanan:code:statictwist}.
...
Figure~\ref{narayanan:code:statictwist}), which derives from the base
...
\caption{Problem definition: The static twisting of a hyperelastic
  cube.}
\label{narayanan:code:statictwist}
\end{figure}

In order to solve this problem, an instance of the {\tt Twist} class
is created and its {\tt solve()} method is called (see
Figure~\ref{narayanan:code:solveproblem}). This triggers a Newton
solve which exhibits quadratic convergence (see
Table~\ref{narayanan:table:residualnorms}) and results in the
displacement field shown in
Figure~\ref{narayanan:fig:twistedcube}.
...
\label{narayanan:code:solveproblem}
\end{figure}
...
\begin{table}
  ...
  \label{narayanan:table:residualnorms}
\end{table}
...
Figure~\ref{narayanan:code:dynamicrelease} are straightforward, and
...
\label{narayanan:code:dynamicrelease}
\end{figure}
...
Figure~\ref{narayanan:fig:releasedcube}. Figure~\ref{narayanan:fig:energies}
...
\begin{figure}[ht]
  \begin{center}
    \subfigure[$t = 0.0$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_000.png}
    }
    \subfigure[$t = 0.1$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_100.png}
      }
    \subfigure[$t = 0.2$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_200.png}
    }
    \subfigure[$t = 0.3$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_300.png}
    }
    \subfigure[$t = 0.4$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_400.png}
      }
    \subfigure[$t = 0.5$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/release_500.png}
    }
    \caption{Relaxation and subsequent re-twisting of a released cube
      over the first 0.5~s of the calculation.}
  \end{center}
  \label{narayanan:fig:releasedcube}
\end{figure}
 
\begin{figure}[ht]
  \begin{center}
    \includegraphics[scale=10]{chapters/narayanan/images/energies.pdf}
  \end{center}
...
the code listing for this problem (Figure~\ref{narayanan:code:%
...
\caption{Problem definition: A hyperelastic dolphin being pushed to
  the right.}
\label{narayanan:code:fishyflow}
\end{figure}
...
method. This is shown in Figure~\ref{narayanan:code:manualstep}.
...
\label{narayanan:code:manualstep}
\end{figure}
...
Figure~\ref{narayanan:code:manualstep} is set up to individually step
...
Figure~\ref{narayanan:fig:tumblingdolphin} shows time snapshots of the
...
\begin{figure}[ht]
  \begin{center}
    \subfigure[$t = 0.0$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_000.png}
    }
    \subfigure[$t = 0.125$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_001.png}
      }
    \subfigure[$t = 0.250$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_002.png}
    }
    \subfigure[$t = 0.375$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_003.png}
    }
    \subfigure[$t = 0.500$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_004.png}
      }
    \subfigure[$t = 0.625$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_005.png}
    }
    \subfigure[$t = 0.750$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_006.png}
    }
    \subfigure[$t = 0.875$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_007.png}
      }
    \subfigure[$t = 1.000$ s]{
      \includegraphics[width=0.3\textwidth]{chapters/narayanan/images/dolphin_008.png}
    }
  ...
  \end{center}
  \label{narayanan:fig:tumblingdolphin}
\end{figure}
