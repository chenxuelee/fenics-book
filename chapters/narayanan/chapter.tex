...
unit volume. Along with \eqref{narayanan:eq:balance-of%
...
\eqref{narayanan:eq:balance-of-momentum} is valid for all
...
\eqref{narayanan:eq:secondpk}, \twist{} computes the second
...
\eqref{narayanan:eq:balance-of-momentum} with a test function 
...
form~\eqref{narayanan:eq:weakform}, by looking for solutions in a finite
...
$\hat{\mathrm{V}^{h}} \subset \hat{\mathrm{V}}$.\footnote{We now
  ...
  \eqref{narayanan:eq:weakform}, along with the various fields
...
\eqref{narayanan:eq:staticweakform} is just a nonlinear functional in
...
\eqref{narayanan:eq:weakform}. The first of these is the
so-called cG(1) method (\cite{ErikssonEstepHansboEtAl1996}). In order
to derive this method, \eqref{narayanan:eq:weakform}, which is
...
\eqref{narayanan:eq:cg1weakformapprox}, and results in a mixed system
...
The CG(1) scheme defined in \eqref{narayanan:eq:cg1weak%
...
\eqref{narayanan:eq:hhtdefs} into the following form of the
...
\eqref{narayanan:eq:newaccn} is then used in the
definitions~\eqref{narayanan:eq:hhtdefs} to update to new displacement
