...
fields which are parametrised by reference positions. This is commonly
...
satisfied\footnote{It should be noted that the story is not
  ...
  when modelling (nearly) incompressible materials, it is well known
  that the ill-conditioned stiffness matrix resulting from the
  conventional Galerkin approximation (discretising only the
...
\item Differentiating forms with respect to coefficients of a discrete
  function, allowing for automatic linearisation of nonlinear
  variational forms:\\
...
method. In case one wants to override this behaviour, they can do so
