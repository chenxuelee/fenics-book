...
elements. We thus bypass elements defined on quadrilaterals and
hexahedra, composite and macro-element techniques, as well as
XFEM-type methods. Even among polynomial-based elements on simplices,
the list of elements can be extended. Nonetheless, this chapter
presents a comprehensive collection of some the most common, and some
more unusual, finite elements.

%------------------------------------------------------------------------------
\section{The finite element definition}

The Ciarlet definition of a \emph{finite element} was first introduced
in a set of lecture notes by~\cite{Ciarlet1975} and became popular after
his 1978 book~\cite{Ciarlet1978, Ciarlet2002}. It remains the standard
definition today, see for example~\cite{BrennerScott2008}.

\begin{definition}[Finite element~{\cite{Ciarlet1975}}]
...
Similar ideas were introduced earlier in~\cite{CiarletRaviart1972}, in
...
was also discussed in~\cite{BrambleZlamal1970}, although the term
...
only if it is continuous~\cite[Theorem 5.2]{Braess2007}. We consider
...
\emph{Courant} triangle, after the seminal paper by~\cite{Courant1943}
...
discrete stability properties; see~\cite{TaylorHood1973,
  ScottVogelius1985}.
...
see~\cite{Warburton2005}, for the purposes of this chapter the points
...
from~\cite{BrennerScott2008} that
\begin{equation}
...
in~\cite{CrouzeixRaviart1973} as a technique for solving the
...
elasticity, see~\cite{HansboLarson2003}, and Reissner--Mindlin plates,
see~\cite{ArnoldFalk1989}.
...
linear Lagrange element~\cite[Chapter 3.I]{Braess2007}:
\begin{equation}
...
higher odd degrees ($q=3,5,7\,\ldots$), see~\cite{CrouzeixFalk1989}.
...
by~\cite{RaviartThomas1977}. It was the first element to discretize
...
by~\cite{Nedelec1980}. It is therefore sometimes referred to as the
...
by~\cite{Nedelec1980} (and~\cite{BrezziFortin1991}). The original
...
that~\cite[Chapter III.3]{BrezziFortin1991}
\begin{equation}
...
The Brezzi--Douglas--Marini element was introduced by Brezzi, Douglas
and Marini in two dimensions (for triangles)
in~\cite{BrezziDouglasMarini1985}. The element can be viewed as an
alternative to the Raviart--Thomas element using a complete polynomial
space. It was later extended to three dimensions (tetrahedra, prisms
and cubes) in~\cite{Nedelec1986}
and~\cite{BrezziDouglasDuranFortin1987}. The definition given here is
based on that of~\cite{Nedelec1986}.
...
tensor; see~\cite{FarhloulFortin1997, ArnoldFalkWinther2007}.
...
\end{equation}
Letting $\Pi_T^q$ denote the interpolation operator defined by the
degrees of freedom for $q = 1, 2, \dots$, we have that~\cite[Chapter
...
rectangles by~\cite{BrezziDouglasFortinEtAl1987}, and the triangular
analogue was given in~\cite{BrezziFortin1991}. In similar spirit,
elements with differing degrees on the boundary suitable for varying
the polynomial degree between triangles were derived
in~\cite{BrezziDouglasMarini1985a}.

\subsection{The Mardal-Tai-Winther element}

The Mardal--Tai--Winther element was introduced
in~\cite{MardalTaiWinther2002} as a finite element suitable for the
...
to three dimensions in~\cite{TaiWinther2006}, but we only present the
...
by~\cite{ArnoldWinther2002}. This paper presented the first stable
...
in~\cite{AdamsCockburn2005} and~\cite{ArnoldAwanouWinther2008}, but we
...
see~\cite{Nedelec1980,Nedelec1986}. For a piecewise polynomial to be
...
introduced in the papers~\cite{Nedelec1980} and~\cite{Nedelec1986}.
...
in~\cite{ArnoldFalkWinther2006}, where the first kind spaces appear as
...
in~\cite{ArnoldFalkWinther2006c}.
...
Letting $\Pi_T^q$ denote the interpolation operator defined by the
degrees of freedom above, we have that~\cite[Theorem 2]{Nedelec1980}
\begin{equation}
...
Letting $\Pi_T^q$ denote the interpolation operator defined by the
degrees of freedom above, we have that~\cite[Proposition 3]{Nedelec1986}
\begin{equation}
...
The Argyris element~\cite{ArgyrisFriedScharpf1968,Ciarlet2002} is
...
(quintic) Argyris elements are~\cite[Chapter II.6]{Braess2007}:
\begin{equation}
...
affinely mapped. Recent work by~\cite{DominguezSayas2008} develops a
...
continuity at the vertices; see~\cite{SolinSegethDolevzel2004}.
...
see~\cite{Ciarlet2002}. Hermite-type elements appear in the finite
element literature almost from the beginning, appearing at least as
early as the classic paper by~\cite{CiarletRaviart1972}. They have
long been known as useful $C^1$-nonconforming elements;
see~\cite{Braess2007,Ciarlet2002}.  Under affine mapping, the Hermite
elements form \emph{affine-interpolation equivalent} families;
see~\cite{BrennerScott2008}.
...
degree; see~\cite{BrennerScott2008}

\subsection{The Morley element}


The Morley triangle defined in~\cite{Morley1968} is a simple
...
by~\cite{Morley1968,Morley1971}. In the mathematical literature,
\cite{LascauxLesaint1975} considered it in the context of the patch
test in a study of plate-bending elements. Recent applications of the Morley element include
\cite{HuangGuoShi2008,MinXu2006}.
...
see~\cite{ArnoldBrezziDouglas1984, ArnoldBrezziFortin1984}. Bubble
...
\item[\emph{The MINI element for the Stokes equations.}]
  ...
  approximation; see~\cite{ArnoldBrezziFortin1984}.
\item[\emph{The PEERS element for weakly symmetric linear elasticity.}]
  ...
  element~\cite{ArnoldBrezziDouglas1984}.
\end{itemize}
...
in~\cite{ArnoldFalkWinther2006}. In finite element exterior calculus,
