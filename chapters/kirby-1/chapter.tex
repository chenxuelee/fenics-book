...
brought a new wave of elements with improved properties, see~\cite{ArnoldFalkWinther2006} for an overview.  A generative
...
\end{equation}
where $x_i$ are the vertices (0,0), (1,0), (0,1) and $\delta$ is the delta function.  The corresponding basis
...
\begin{figure}
  \begin{center}
    ...
    \caption{Triangular Raviart--Thomas elements of order zero.}
    \label{RaviartThomas}
  \end{center}
\end{figure}
...
\begin{equation}
\left\{
u \in (\P{n}^2 (T) )^2 : u \cdot n \in \Poly{n-1}^1(E_i), E_i \in \mathcal{E}(T)
\right\},
\end{equation}
