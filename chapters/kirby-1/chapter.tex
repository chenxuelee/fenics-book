...
$\mathbf{xx}\in T$ and $\mathbf{xx}i\in\hat T$ we use the mapping 
\begin{equation}
\label{eq:geometry}
\mathbf{xx} = \GG(\mathbf{xx}i) + \mathbf{xx}_0,
\end{equation}
and define the Jacobian determinant of this mapping as
\begin{equation}
\label{eq:geometry2}
J(\mathbf{xx}) = \left| \frac{\partial\GG(\mathbf{xx}i) }{\partial \mathbf{xx}i} \right| .
\end{equation}
The basis functions are defined in terms of the basis function
on the reference element as
\begin{equation}
\label{eq:subs}
N_j(\mathbf{xx}) = \hat{N_j}(\mathbf{xx}i), 
\end{equation}
...
\int_T \, f(\mathbf{xx}) \, d\mathbf{xx} = \int_{\hat{T}} \, f(\mathbf{xx}i) \, J d\mathbf{xx}i ,
\end{equation}
