...
constructed on top of DOLFIN~\citep{LoggWells2010}, is
...
\item Based on the behavior of the Brezzi constants with respect to
  ...
  see~Definitions~\ref{rognes:def:stable} and~\ref{rognes:def:reduced_stable}, as
  \begin{enumerate}
  \item Stable
  \item Unstable
...
DOLFIN for the specification of forms, inner products and
...
\end{python}
A \emp{StabilityResult} is returned.  The instructions carried out
by this function and the properties of the \emp{StabilityResult}
...
of \emp{compute\_brezzi\_infsup}. The input consists of
the form $b$, the inner products $(m, n)$, and a function space $W_h$
(and optionally, an essential boundary
condition \emp{bc}). The aim is to construct the
...
hand sides (\emp{lhs} and \emp{rhs}). If only a single form $c$ and a
...
An \emp{InfSupConstant} represents a single inf-sup
...
Finally, the \emp{StabilityResult} class holds a list of
possibly several \emp{InfSupCollections}, each
corresponding to a separate inf-sup condition, such as the Brezzi
coercivity and the Brezzi inf-sup condition. The
\emp{StabilityResult} identifies a discretization as
