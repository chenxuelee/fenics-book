...
The goal of this note is to demonstrate that the process of
...
\begin{figure}%[ht]
  \begin{center}
    \subfigure[Bad approximation]{
      \includegraphics[width=0.46\textwidth]{chapters/rognes/eps/CG.eps}
      \label{rognes:fig:cg}
    }
    \subfigure[Good approximation]{
      \includegraphics[width=0.46\textwidth]{chapters/rognes/eps/RT.eps}
      \label{rognes:fig:rt}
      }
...
For a Hilbert space $\VV$, we denote the norm on $\VV$ by
...
The condition~\eqref{rognes:eq:infsup:discrete} has a simple interpretation
...
We begin by considering the Babu\v ska inf-sup constant for the element
...
%% In conclusion, the three generalized eigenvalue
%% problems~\eqref{rognes:eq:eigenvalue:babuska},
%% \eqref{rognes:eq:eigenvalue:coercivity}, \eqref{rognes:eq:eigenvalue:infsup} can be
%% used to compute the Babuska, Brezzi coercivity and Brezzi inf-sup
%% constants respectively, given bases for the spaces $V_h$ and
%% $Q_h$. Let $\{\phi_i\}_i \subset V_h$ and $\{ \psi_j \}_j \subset Q_h$
%% be such bases. We define the matrices
%% \begin{equation}
%%   \label{rognes:eq:matrices}
%%   \begin{split}
%%     A_{ij} &= a(\phi_i, \phi_j) \quad
%%     B_{ij}^T = b(\phi_i, \psi_j) \\
%%     M_{V, ij} &= \langle \phi_i, \phi_j \rangle_{V} \quad
%%     M_{Q, ij} = \langle \psi_i, \psi_j \rangle_{Q}
%%   \end{split}
%% \end{equation}
%% The three eigenvalue problems to be used in the following, expressed
%% with reference to the above bases and corresponding element matrices,
%% are summarized in Table~\ref{rognes:tab:eigenvalue_problems}.
%% \begin{table}
%%   \centering
...
%%       \begin{description}
%%         \item[Babu\v ska inf-sup] $\gamma_h = |\lambda_{\min}|$ of
%%         \begin{align*}
%%           A u + B^T p &= \lambda M_V u \\
%%           B u  &= \lambda M_Q p
%%         \end{align*}
%%         \item[Brezzi coercivity] $\alpha_h = |\lambda_{\min}|$ of
%%         \begin{align*}
%%           A u + B^T p &= \lambda M_V u \\
%%           B u  &= 0
%%         \end{align*}

%%         \item[Brezzi inf-sup] $\beta_h = \sqrt{\lambda_{\min}}$ of
%%         \begin{align*}
%%           M_V u + B^T p &= 0 \\
%%           B u  &= - \lambda M_Q p
%%         \end{align*}
...
%%   \caption{The three generalized eigenvalue
%%     problems~\eqref{rognes:eq:eigenvalue:babuska},
%%     \eqref{rognes:eq:eigenvalue:coercivity}, \eqref{rognes:eq:eigenvalue:infsup}
%%     summarized and stated in terms of finite element bases. $A, B,
%%     M_V$ and $M_Q$ are as defined in~\eqref{rognes:eq:matrices}.}
%%   \label{rognes:tab:eigenvalue_problems}
%% \end{table}
...
Let us take a closer look at the implementation of
...
A collection of \textrm{InfSupConstant}s forms an
...
When the above is applied to the diagonal mesh family, the following
