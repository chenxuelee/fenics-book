...
point $x\in\omega(t)$; that is, $X \mapsto x = \Phi(X,t)$.
\begin{figure}
...
Oslo. Currently, \emp{CBC.Solve} consists of two core components;
\emp{CBC.Flow} and \emp{CBC.Twist} which are frameworks explicitly
...
\addtolength{\fboxsep}{5pt} \boxed{
\begin{split}
E_h &\leqslant \sum_{n=1}^{N}\int_{I_n} \sum_{T\in\mathcal{T}_h}| \langle
R_T,\; Z - \pi_h Z \rangle_T |+ | \langle \llbracket R_{\partial T}
\rrbracket,\; Z - \pi_h Z \rangle _{\partial T}|\dt\\\\ E_k &\leqslant
...
we refine the mesh until $E_h \leqslant \mathrm{TOL}_h$.  There are various
