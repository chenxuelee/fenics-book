...
using the \emp{cbc.blocks} and the Python interfaces of DOLFIN and Trilinos\index{Trilinos}.
...
operators as mappings in properly chosen Sobolev spaces, before giving a short 
overview of \emp{cbc.blocks}. We then
...
When the explicit matrix product is required, such as for input to the ML preconditioner, a method \emp{collapse} is provided that performs the calculations using PyTrilinos. This method requires that all components are actual matrices, not general operators.
...
can be developed relatively easily by using \emp{cbc.blocks} and the Python interfaces of
DOLFIN and Trilinos. The \emp{cbc.blocks} module allows rather complicated preconditioners to be
