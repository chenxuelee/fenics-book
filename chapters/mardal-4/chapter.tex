...
package ML which is contained in PyTrilinos\index{PyTrilinos}. Furthermore, we will estimate the eigenvalues
...
shown above. We use test different refinements of the unit square and use
continuous piecewise linear elements, $\mathrm{CG}_1$. The source function $f= 500\exp(-((x-0.5)^2 + (y-0.5)^2)/0.02)$ and
$g =  25 \sin(5\pi y)$ for $x=0$ and zero elsewhere, see also the source code \emp{poisson\_neumann.py}. 
\begin{table}
\begin{center}
...
$H^1_g \times L_0^2$ to its dual $H_g^{-1} \times L_0^2$, see e.g.~\cite{Brezzi,BrezziFortin}. Therefore, we 
...
For an overview of similar and alternative preconditioning techniques cf. eg.~\cite{B-G-L-05,E-S-W-text}. 
