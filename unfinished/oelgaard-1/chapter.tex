...
$\Omega$ will be denoted by~$\mathcal{T}$, and a triangulation of the
domain $\Omega_{0}$ will be denoted by~$\mathcal{T}_{0}$.  A finite
element cell will be denoted by $K \in \mathcal{T}$.
...
\Omega \times I \rightarrow \mathbb{R}^{d}$ is the initial velocity.
...
with time derivatives.  Following this approach, at a time $t \in (0,
T]$ we multiply \eqref{eq:oelgaard-1:gov_balance_momentum} by a function
...
the boundary condition in \eqref{eq:oelgaard-1:gov_neumann_bc}, we obtain:
%
\begin{equation}
...
\begin{equation}
  ...
  \phi\brac{\sigma, q_{\rm kin}\brac{\varepsilon^{\rm p}}} - q_{\rm iso}\brac{\kappa} - \sigma_{\rm y} \leq 0,
...
In the flow theory of plasticity, plastic strain rate is given by:
%
\begin{equation}
...
algorithms for to compute the stress from increments of the total strain.
...
strain energy function $\Psi_{0}$. The linearized model presented at
...
models, features of \ufl{} will permit problems to be posed as energy
...
space, sub-optimal convergence of a Newton method will be observed.

%------------------------------------------------------------------------------
\subsection{Consistency of linearization}

To illustrate the second issue raised in the preceding paragraph, we
consider the representation that FFC would generate for a simple model
...
\end{equation}
%
where $q$ is integration point index, $\alpha$ is the degree of freedom
...
exhibit sub-optimal convergence.  In general, the illustrated problem
...
in the following section, as too is a solution that involves the dentition
of `quadrature elements'.
...
%
\begin{equation}
  V = \left\{ w \in H^{1}_{0}(\Omega), \ w \in P_{k}(K) \ \foralls K \ \in \mathcal{T} \right\},
\end{equation}
%
where $\Omega \subset \mathbb{R}$ and $k \ge 1$, the model problem of
...
process is not consistent with the problem being solved.

To resolve this issue within the context of the Unified Form Language
(UFL, Chapter~\ref{chap:alnes-1}), and the FeniCS Form Compiler (FFC,
Chapter~\ref{chap:logg-1}), the concept of \emph{quadrature elements}
\index{quadrature element} has been developed.  This special type of
element is used to represent `functions' that can only be evaluated at
particular points (quadrature points), and cannot be differentiated.
In the remainder of this section we present some key features of the
quadrature element and a demonstration of its use for the model problem
...
\end{python}
%
where \emp{k} is the polynomial degree that the underlying quadrature
...
$q$ that is defined on a quadrature element:
%
\begin{equation}
  a\brac{u, w} = \int_{\Omega} q u w \dx.
...
This means that the function $q$ should be defined as:
%
\begin{python}
ElementQ = FiniteElement("Quadrature", tetrahedron, 2)
q = Coefficient(ElementQ)
\end{python}
%
...
element, as described above, only have non-zero values at points that
...
declaration for the coefficient $q$ should be changed to:
%
\begin{python}
...
Continuous, discontinuous and quadrature elements are denoted by $CG_{k}$,
$DG_{k}$ and $Q_{k}$ respectively where $k$ refers to the polynomial
...
The examples are chosen to highlight some implementation aspects that are
...
problem in presented in Figure~\ref{fig:oelgaard-1:elas_code}.
...
means that that the expressions for \emp{sigma} and \emp{F} in
...
to range of plasticity models that are defined by the form of the
...
In addition to the \ufl file, which defines the momentum balance
equation for a plasticity problem, two auxiliary \ufl files
are required. The first is used to compute the strain from
the displacement field via an
$L^2$-projection\footnote{It would be more efficient to compute
the strain at quadrature points directly from the displacement
field, but this is not presently supported. The projection approach works
for elements for which it is possible to define a lower-dimensional
space that coincides exactly with the derivative of the original
space. This is the case for Lagrange basis functions on simplices.
The cost of the projection operation is small relative to other
operations in the solution process for the plasticity problem.}
as shown in
Figure~\ref{fig:oelgaard-1:plas_strain_code}, and the second defines
the function space for the equivalent plastic strain field, and which
is shown in Figure~\ref{fig:oelgaard-1:plas_eqstrain_code}.
...
s, w = TrialFunction(elementS), TestFunction(elementS)
...
L = inner(eps(u), w)*dx
\end{python}
\caption{Definition of the linear and bilinear variational forms to
compute strains via an $L^2$-projection of the displacement field.
Note that the \emp{eps} function returns engineering strain.}
\label{fig:oelgaard-1:plas_strain_code}
\end{figure}

\begin{figure}
\begin{python}
element = FiniteElement("Quadrature", tetrahedron, 2)
\end{python}
\caption{Definition of the function space for the equivalent plastic strain
field.}
\label{fig:oelgaard-1:plas_eqstrain_code}
\end{figure}
...
First, the necessary function spaces are created.  \emp{Vps} and
\emp{Veq} are used for the two history variables, the plastic strain
and equivalent plastic strain.  \emp{Vt} and \emp{Vs} are used for
the two coefficient spaces: the consistent tangent and the stress,
which enter the bilinear and linear forms of the plasticity problem.
The function space \emp{Vs} is also used for the strains.  The forms
defining the plasticity problem and the forms to compute the strains
are then created and the relevant functions are attached to the forms.
Then the object defining the plasticity model is created.  The class
\emp{VonMises} is a sub-class of the \emp{PlasticityModel} class shown
...
with the other necessary forms, function spaces, coefficient functions
and boundary conditions.  \emp{PlasticityProblem} is a sub-class
...
Plasticity::LinearForm::CoefficientSpace_s Vps(mesh);
Plasticity::BilinearForm::CoefficientSpace_t Vt(mesh);
Strain::FunctionSpace Vs(mesh);
EQPlasStrain::FunctionSpace Veq(mesh);
...
Function u(V);
Strain::BilinearForm a_strain(Vs, Vs);
Strain::LinearForm L_strain(Vs);
L_strain.u = u;
...
fenicsplas::PlasticityProblem nonlinear_problem(a, L, a_strain, L_strain, Vps, Veq, tangent, stress, bcs, J2);
...
therefore as simple as redefining the stored energy function~$\Psi_{0}$.
...
that $u=0$ at $x=0$ and the initial conditions are $u_{0} = v_{0} = 0$.
