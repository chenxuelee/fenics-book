...
the fact that we create an integral object using the function \emp{integral}, but we 
need to explicitly call the function \emp{eval\_integ} to compute the integral. In a similar
fashion one may use functions like \emp{eval}, \emp{evalm}, \emp{expand}, \emp{simplify}, 
...
For instance, Swiginac unwraps the \emp{ex} objects and provides typemaps between Python lists and GiNaC
lists (\emp{lst}). 
...
\emp{-ogenerate\_code} means that the generated code should be located
in the directory \emp{generate\_code}. Notice that the flags and
corresponding options are not separated by whitespaces. A complete
list of options is obtained with \emp{sfc -h}.  
...
Earlier versions of SFC were produced slow code for complicated nonlinear
...
class CellIntegralRepresentation(IntegralRepresentation):
    ...
    def compute_A(self, data, iota, facet=None):
        ...
        if data.integration_method == "quadrature":
            ...
            ...     
\end{python}
The representation classes are quite involved, in particular when using
quadrature since this is where the 
graph structures (\emp{Graph} in UFL) are used to split the expressions 
in smaller sub-expressions. 
SFC makes GiNaC symbols that represent temporary variables for all
sub-expressions. Based on the partition from UFL, the temporary variables
