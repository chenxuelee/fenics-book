...
as~\cite{AinCoy02}.  These approaches can provide greater flexibility for
...
\int_T \, f(\mathbf{x}) \, d\mathbf{x} = \int_{\hat{T}} \, f(\xi) \, J d\xi ,
\end{equation}
...
\frac{\partial \phi}{\partial \xi_j} \frac{\partial \xi_j }{\partial x_i }  .
\end{equation}
...
basis \( \left\{ \psi \right\}_{i=1}^{\dim P} \subset P \).  From
this, we construct each nodal basis function
\begin{equation}
 \phi_j = A_{jk} \psi_k,
\end{equation}
where summation is implied over the repeated index \( k \).  The
task is to compute the matrix \( A \).  Each fixed \( \phi_j \) must
satisfy
\begin{equation}
L_i( \phi_j ) = \delta_{i,j},
\end{equation}
...
\begin{equation}
\delta_{i,j} = L_i( A_{jk} \psi_{k} ) = A_{jk} L_i (\psi_k).
\end{equation}
...
\( \{ x^i \}_{i=0}^{n} \), so that any \( \psi \in P_n \) is written as
\begin{equation}
\label{pn1d}
\psi = a_0 + a_1 x + \ldots a_n x^n = \sum^n_{i=0} a_i x^i.
\end{equation}
...
\[
\psi_k(x) = \frac{1}{2^k k!} \frac{d^k}{dx^k} (x^2 -1), \quad k=0,1,\ldots,
\]
...
\[
\psi_{k,l,m}(x,y,z) = \psi_k(x) \psi_l(y) \psi_m(z) .
\]
and $\P^n$ is defined by functions on the form (in 3D),
\[
v = \sum_{k,l,m=0}^{k,l,m <= n}   a_{k,l,m} \psi_{k,l,m} .
\]
...
that
\( \{ \psi_i \}_{i=1}^{\dim n} \) forms a basis for  polynomials of
...
\[
\psi_{i,n} = \binom{i}{n} x^i (1-x)^{n-i}, \quad i=0,\ldots,n,
\]
and then \( P_n \) is spanned by \( \{ \psi_{i,n} \}_{i=0}^n \)
