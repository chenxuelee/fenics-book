...
projects.  Our goal is to provide a framework by which simplicial finite elements
...
are becoming widely popular in isogeometric analysis~\cite{HughesCottrellBazilevs2005}, nor does
it entirely address XFEM~\cite{ChessaSmolinksiBelytschko2002} or {\em hp}-type methods~\cite{Schwab1998}.  However, in isogeometric
...
tetrahedral $hp$ bases are available~\cite{AinsworthCoyle2003}.  We
...
\texttt{ffc}~\citep{Logg2007b} and \texttt{sfc}~\citep{AlnaesMardal2009} will work
...
and Bernstein bases, and orthogonal bases such as Dubiner, and Jacobi.  Each of these bases
has explicit representations or reccurence relations making them easy to 
evaluate and differentiate. In contrast,  
most finite element bases that are determined by solving 
...
On a line segment, $\P_q^1=\P_q$ the monomial, or power basis is
...
\end{split}
\end{equation}
with a three-term recurrence for $n\geq2$:
\begin{equation}
\label{eq:recur}
P_{n+1}^{\alpha,\beta}(x) 
= (a^{\alpha,\beta}_n x + b^{\alpha,\beta}_n) P_n^{\alpha,\beta}(x) - c^{\alpha,\beta}_n P_{n-1}^{\alpha,\beta}(x),
\end{equation}
...
elements~\citep{missing}, is known under many names in the literature.  It is
...
\begin{figure}
\begin{center}
\includegraphics[height=3cm]{chapters/kirby-1/pdf/tricoord.pdf} 
\end{center}
...
More recently, Kirby~\cite{Kirby2009,Kirby2010} has found special
structure among finite element operators based on Bernstein
...
longer be direct.  See~\cite{Kirby2004} for more details.
...
\end{equation}
Here, \( V^1 \in \mathbb{R}^{2 \kappa - 3, 2\kappa} \) and
\( V^2 \in \mathbb{R}^{3, 2 \kappa} \) are defined
by
\[V^1_{ij} = L_i( \phi_j ),
\]
\[
...
a readily usable basis.  
If we may characterize \( P \) by
\[
P = \cap_{i=1}^{\dim{\bar{P}}-\dim P} \ell_i,
\]
where \( \ell_i : \bar{P} \rightarrow \mathbb{R} \) are linear
functionals, then we may apply this technique.  
In this case, let $\kappa = \dim P$ and $\bar{\kappa}
= \dim{\bar{P}}$.  Now,
\( V_1 \in \mathbb{R}^{\kappa,\bar{kappa}} \) and
\( V_2 \in \mathbb{R}^{\bar{\kappa}-\kappa,\bar{kappa}} \).  This
...
Let \( \{ \phi_i \}_{i=1}^{\bar{\kappa}} \) be a basis for \( \bar{P}
\).  Suppose that there exist functionals 
\( \{ \ell_i \}_{i=1}^{\bar{\kappa} - \kappa} \) on \( \bar{P} \) such that
\[
P = \cap_{i=1}^{\bar{\kappa} - \kappa} \mathrm{null}(\ell_i).
\]
...
\psi_i = A_{ij} \phi_j,
\]
where \( 1 \leq i \leq \kappa \) and \( 1 \leq j \leq \bar{\kappa} \).
\end{proposition}

\section{Operations on the Polynomial spaces}
Here, we show how various important operations may be cast
in terms of linear algebra, supposing that they may be performed on
...
details on the Dubiner basis, see~\citep{missing}.  Then, \( \Psi^\alpha \)
may be computed by
\[
...
in some cases so-called sparse rules may win~\cite{KeeganRidzalBochev2008}
...
\end{equation}
For isoparametric elements, the basis functions are defined in terms of the basis functions
...
is no longer the same as the geometry mapping. That is, for $\Hdiv$ elements, the Piola mapping reads  
\begin{equation}
\label{eq:subs}
...
This numbering has also been discussed in \citet{ArnoldFalkWinther2006}. 
...
The finite element needs to provide information on how the elements should be patched together, 
whether their degrees of freedom are shared with neighbouring elements or not, see Figure~\ref{fig:kirby-1:patch}.   
To obtain the desired patching, the elements should provide unique identifiers determining their global degree of freedom. 
Most common perhaps, is to relate each degree of freedom on the reference cell to a point in the
reference cell. The geometry mapping then gives a global point in the mesh, given by \eqref{eq:geometry}, that identifies the degree of freedom. 
Elements sharing points are then sharing the corresponding degrees of freedom.   
Alternatively, each degree of freedom may be related to a local mesh entity, like a vertex, edge or face, on the reference element. After mapping the element, the degree of freedom will then be related to the corresponding mesh entity in the global mesh.      
