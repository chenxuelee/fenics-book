...
%\newcommand{\langtangenidx}[1]{\index{#1@\texttt{#1}}}
...
%\newcommand{\hpl}[1]{{\bf hpl comment: }{\itshape #1}}
\newcommand{\hpl}[1]{}
...
The Poisson equation \eqref{langtangen:poisson1} arises in numerous physical
...
Having a mesh, we can define a discrete function space {\fontsize{10pt}{10pt}\texttt{V}} over this mesh:
...
\index{finite element specifications}\index{CG@\langtangencodett{CG} finite element family}\index{Lagrange@\langtangencodett{Lagrange} finite element family}
...
program:\langtangenidx{TestFunction}\langtangenidx{TrialFunction}
\begin{python}
v = TestFunction(V)
u = TrialFunction(V)
\end{python}
...
One can also omit the {\fontsize{10pt}{10pt}\verb!on_boundary!} argument,
...
Having {\fontsize{10pt}{10pt}\verb!a!} and {\fontsize{10pt}{10pt}\verb!L!} defined, and information about essential
...
This process is known as interpolation.\index{interpolation}\langtangenidx{interpolate}
...
To demonstrate the use of point evaluations of {\fontsize{10pt}{10pt}\verb!Function!} objects,
...
{\fontsize{10pt}{10pt}\texttt{pydoc dolfin.Mesh}}\langtangenidx{pydoc} in a terminal window
...
We have seen how to extract the nodal values in a {\fontsize{10pt}{10pt}\texttt{numpy}} array.
...
%We have commented out the {\fontsize{10pt}{10pt}\texttt{plot}} and {\fontsize{10pt}{10pt}\texttt{interactive}} calls in
%this version of the program, but if you want plotting to happen, make
%sure that {\fontsize{10pt}{10pt}\texttt{interactive}} is called at the very end of the program.
...
The program {\fontsize{10pt}{10pt}\verb!Poisson2D_D3.py!} extends the
...
can be replaced by a one-line call:\langtangenidx{project}\index{projection}
\begin{python}
...
The square in the expression for {\fontsize{10pt}{10pt}\texttt{error}} will be expanded and lead
...
Calling {\fontsize{10pt}{10pt}\texttt{compute}} for finer and finer meshes enables us to
...
The relevant statements read\langtangenidx{assemble}\index{linear systems (in FEniCS)}\index{assembly of linear systems}
\begin{python}
A = assemble(a)
b = assemble(L)
bc.apply(A, b)
...
the {\fontsize{10pt}{10pt}\texttt{Matrix}} and {\fontsize{10pt}{10pt}\texttt{Vector}} objects as follows:\langtangenidx{File}
\begin{python}
...
The program {\fontsize{10pt}{10pt}\verb!Poisson2D_Dvc.py!} contains in addition some plots,
...
Let us go back to the {\fontsize{10pt}{10pt}\verb!Poisson2D_Dvc.py!} code from
...
Other functions for visualizing 2D scalar fields are {\fontsize{10pt}{10pt}\texttt{surf}} and
...
\label{langtangen:poisson:2D:fig2}
  \centerline{
\subfigure[]{\includegraphics[width=0.9\linewidth]{chapters/langtangen/pdf/Poisson2D_Dvc_contour1.pdf}}}
  \centerline{
\subfigure[]{\includegraphics[width=0.8\linewidth]{chapters/langtangen/pdf/Poisson2D_Dvc_flux_x.pdf}}
}
  \caption{
...
\label{langtangen:poisson:2D:fig3}
  \centerline{
\subfigure[]{\includegraphics[width=0.9\linewidth]{chapters/langtangen/pdf/Poisson2D_Dvc_surf1.pdf}}}
  \centerline{
\subfigure[]{\includegraphics[width=0.9\linewidth]{chapters/langtangen/pdf/Poisson2D_Dvc_mesh1.pdf}}}
  \caption{
  Examples on plots created by transforming the finite element field to
  a field on a uniform, structured 2D grid:
...
%See Section~\ref{sec:abstract,nonlinear}!
...
The file {\fontsize{10pt}{10pt}\verb!nlPoisson_Picard.py!} contains the complete code for
...
The {\fontsize{10pt}{10pt}\texttt{F}} form corresponding to \eqref{langtangen:poisson:nonlinear2}
...
The examples in Section~\ref{langtangen:fundamentals}
...
The following modifications are needed in the {\fontsize{10pt}{10pt}\verb!diffusion2D_D1.py!}
...
\end{python}
For example, in 2D {\fontsize{10pt}{10pt}\verb!kappa_str[1]!} becomes
\begin{Verbatim}[fontsize=\fontsize{10pt}{10pt},tabsize=8,baselinestretch=0.85,
fontfamily=tt,xleftmargin=7mm]
...
\hpl{How are relative and absolute tolerances actually used? Does it depend on the backend?}
...
means a Python object of a particular type (such as \langtangencodett{SubDomain},
\langtangencodett{Function},
\langtangencodett{FunctionSpace}, etc.).
...
The next task is to use a {\fontsize{10pt}{10pt}\texttt{MeshFunction}} to mark all
...
%the subdomain numbers, e.g., {\fontsize{10pt}{10pt}\texttt{LOWER}} for no.~0 and
%{\fontsize{10pt}{10pt}\texttt{UPPER}} for no.~1 in our case\footnote{It is common to
...
Calling {\fontsize{10pt}{10pt}\texttt{subdomains.values()}} returns a {\fontsize{10pt}{10pt}\texttt{numpy}} array of the
...
Having the {\fontsize{10pt}{10pt}\texttt{k}} function ready for finite element computations, we
...
Class method (Python): a function in a class, reached by dot notation: {\fontsize{10pt}{10pt}\verb!instance_name.method_name!}\gln
...
{\fontsize{10pt}{10pt}\texttt{pydoc}} for Python objects. You can type\langtangenidx{pydoc}
\begin{Verbatim}[fontsize=\fontsize{10pt}{10pt},tabsize=8,baselinestretch=0.85,
fontfamily=tt,xleftmargin=7mm]
pydoc dolfin.X
\end{Verbatim}
...
{\fontsize{10pt}{10pt}\texttt{UnitInterval}}, {\fontsize{10pt}{10pt}\texttt{UnitCube}}, {\fontsize{10pt}{10pt}\texttt{UnitCircle}}, {\fontsize{10pt}{10pt}\texttt{UnitSphere}},
...
{\fontsize{10pt}{10pt}\texttt{TestFunction(V)}}: define a test function on a space {\fontsize{10pt}{10pt}\texttt{V}} to be used
...
{\fontsize{10pt}{10pt}\texttt{interpolate(func, V)}}: interpolate a formula or finite
...
%outdated:
%User-defined functions are implemented as a {\fontsize{10pt}{10pt}\texttt{Function}} object.
%There are several ways of constructing such objects, such as
%\begin{itemize}
%\item a string containing a mathematical expression,
%\item a subclass of {\fontsize{10pt}{10pt}\texttt{Function}} in Python,
%\item a subclass of {\fontsize{10pt}{10pt}\texttt{Function}} in C++,
%\item a set of degrees of freedom of a finite element function.
%\end{itemize}
%Writing {\fontsize{10pt}{10pt}\texttt{pydoc dolfin.Function}} gives a documentation of the various
%techniques.
...
%More flexibility in defining a {\fontsize{10pt}{10pt}\texttt{Function}} object is achieved by
%subclassing {\fontsize{10pt}{10pt}\texttt{Function}} and implementing the {\fontsize{10pt}{10pt}\texttt{eval}} method:
