...
First, all of the implemented C++ operators, just like we did for the \emp{operator+=()} in the \emp{Array} example above. This is done in the \emp{la\_pre.i} file:
\begin{c++}
...
Finally, we first rename the assignment operator to \emp{\_assign}. The \emp{\_assign} operator will be used by
the \emp{slice} operator implemented in \emp{la\_post.i} below.
...
Instead, we create C++ code that inherit from \emp{Expression} and implements the \emp{eval} method. 
